...
function buildVideoAsArticleCard(it) {
  ...
  /** Phase 6.1: payment method resolver – evidence-only cash|card|transfer|unknown. */
  function iuEvidencePaymentMethodResolver(linesText, pipelineResult) {
    ...
    var cash = /\b(hotovost|cash|v\s*hotovosti)\b/i.test(t);
    var card = /\b(karta|card|platba\s*kartou|kartou)\b/i.test(t);
    var transfer = /\b(převod|transfer|bankovní\s*převod)\b/i.test(t);
    var count = (cash ? 1 : 0) + (card ? 1 : 0) + (transfer ? 1 : 0);
    var value = "unknown";
    var resolvedSafely = false;
    var conflict = false;
    if (count === 1) {
      if (cash) { value = "cash"; resolvedSafely = true; }
      else if (card) { value = "card"; resolvedSafely = true; }
      else if (transfer) { value = "transfer"; resolvedSafely = true; }
    } else if (count > 1) { conflict = true; }
    return { paymentMethodValue: value, paymentMethodResolvedSafely: resolvedSafely, paymentMethodConflict: conflict, used: true };
  }

  /** Phase 6.1: field-level certainty metadata. */
  function iuEvidenceFieldCertainty(pipelineResult, candRes, paymentRes) {
    var cf = (pipelineResult && pipelineResult.correctedFields) || {};
    var vs = (pipelineResult && pipelineResult.validationSummary) || {};
    var totalSafe = !!(candRes && candRes.totalResolvedSafely);
    var vatSafe = !!(candRes && candRes.vatResolvedSafely);
    var dateSafe = !!(candRes && candRes.dateResolvedSafely);
    var receiptIdSafe = !!(candRes && candRes.receiptIdResolvedSafely);
    var merchantSafe = !!(cf.store && cf.store !== "unknown");
    return {
      total: { value: cf.total, resolvedSafely: totalSafe, sourceType: totalSafe ? "composite" : "keyword", confidenceBucket: totalSafe ? "high" : (cf.total !== "unknown" ? "medium" : "unknown"), needsReview: !totalSafe, reviewReason: !totalSafe && cf.total !== "unknown" ? "total_uncertain" : null },
      vat: { resolvedSafely: vatSafe, confidenceBucket: vatSafe ? "high" : "unknown", needsReview: !vatSafe },
      date: { value: cf.date, resolvedSafely: dateSafe, sourceType: dateSafe ? "pattern" : "keyword", confidenceBucket: dateSafe ? "high" : "unknown", needsReview: !dateSafe },
      receiptId: { value: pipelineResult && pipelineResult.docNumber, resolvedSafely: receiptIdSafe, confidenceBucket: receiptIdSafe ? "high" : "unknown", needsReview: !receiptIdSafe },
      merchant: { value: cf.store, resolvedSafely: merchantSafe, confidenceBucket: merchantSafe ? "high" : "unknown", needsReview: !merchantSafe },
      paymentMethod: { value: paymentRes && paymentRes.paymentMethodValue, resolvedSafely: !!(paymentRes && paymentRes.paymentMethodResolvedSafely), confidenceBucket: (paymentRes && paymentRes.paymentMethodResolvedSafely) ? "high" : "unknown", needsReview: !!(paymentRes && paymentRes.paymentMethodConflict) }
    };
  }

  /** Phase 6.1: explicit review reasons. */
  function iuEvidenceReviewReasons(validationSummary, candRes, paymentRes, geometry) {
    var reasons = [];
    if (validationSummary && validationSummary.sumMismatch) reasons.push("math_inconsistency");
    if (validationSummary && validationSummary.errors && validationSummary.errors.indexOf("sumMismatch") >= 0) reasons.push("total_conflict");
    if (validationSummary && validationSummary.vatConsistency === false) reasons.push("vat_conflict");
    if (candRes && !candRes.dateResolvedSafely) reasons.push("date_conflict");
    if (candRes && !candRes.receiptIdResolvedSafely) reasons.push("receipt_id_conflict");
    if (paymentRes && paymentRes.paymentMethodConflict) reasons.push("payment_conflict");
    if (geometry && (!geometry.words || geometry.words.length < 2)) reasons.push("weak_geometry");
    if (!candRes || !candRes.totalResolvedSafely) reasons.push("ambiguous_layout");
    return { reviewReasons: reasons, present: true };
  }

  /** Phase 6.1: warranty-ready metadata (no expiry invented). */
  function iuEvidenceWarrantyReadyMetadata(pipelineResult, candRes) {
    var cf = (pipelineResult && pipelineResult.correctedFields) || {};
    var items = (pipelineResult && pipelineResult.items) || [];
    var purchaseDateCandidate = cf.date || (pipelineResult && pipelineResult.rawOcrText ? "unknown" : "unknown");
    var purchaseDateResolvedSafely = !!(candRes && candRes.dateResolvedSafely);
    var receiptIdCandidate = (pipelineResult && pipelineResult.docNumber) || "unknown";
    var merchantResolvedSafely = !!(cf.store && cf.store !== "unknown");
    var itemsLinkedToDocument = items.length > 0;
    var totalResolvedSafely = !!(candRes && candRes.totalResolvedSafely);
    var warrantyReadyMinimum = (purchaseDateResolvedSafely || purchaseDateCandidate !== "unknown") && (merchantResolvedSafely || receiptIdCandidate !== "unknown") && (itemsLinkedToDocument || totalResolvedSafely);
    return {
      purchaseDateCandidate: purchaseDateCandidate,
      purchaseDateResolvedSafely: purchaseDateResolvedSafely,
      receiptIdCandidate: receiptIdCandidate,
      merchantResolvedSafely: merchantResolvedSafely,
      itemsLinkedToDocument: itemsLinkedToDocument,
      warrantyReadyMinimum: warrantyReadyMinimum,
      warrantyReadyMinimumComputed: true,
      warrantyExpiryInvented: false,
      present: true
    };
  }

  /** Phase 6.5: short hash for documentId / variant suffix. */
  function iuEvidenceShortHash(s) {
    var str = (s == null || s === "unknown") ? "u" : String(s).trim();
    var h = 5381;
    for (var i = 0; i < str.length; i++) h = ((h << 5) + h) + str.charCodeAt(i);
    return (h >>> 0).toString(36).slice(0, 8);
  }

  /** Phase 6.5: items fingerprint = hash(sorted rawText) for collision-safe identity. */
  function iuEvidenceItemsFingerprint(items) {
    if (!items || items.length === 0) return "no_items";
    var rawTexts = items.map(function(it) { return (it.rawText != null ? String(it.rawText) : "").trim(); }).filter(Boolean);
    rawTexts.sort();
    var str = rawTexts.join("|");
    if (!str) return "no_items";
    var h = 5381;
    for (var i = 0; i < str.length; i++) h = ((h << 5) + h) + str.charCodeAt(i);
    return "it_" + (h >>> 0).toString(36);
  }

  /** Phase 6.3/6.5: deterministic recordId from merchant, date, total, currency, docType, receiptId, itemsFingerprint (Phase 6.5 hardening). */
  function iuEvidenceRecordIdDeterministic(merchantVal, purchaseDateVal, totalVal, currencyVal, docTypeVal, receiptIdVal, itemsFingerprint) {
    ...
    var fp = (itemsFingerprint == null || itemsFingerprint === "unknown") ? "no_items" : String(itemsFingerprint);
    var parts = [m, d, t, c, doc, r, fp];
    var str = parts.join("|");
  ...
  function iuEvidenceBuildCanonicalPurchaseRecord(pipelineResult, p6, geometry) {
    ...
    var itemsFingerprint = iuEvidenceItemsFingerprint(canonicalItems);
    ...
    var documentId = "doc_" + iuEvidenceShortHash([merchantNorm.value, purchaseDateNorm.value, totalNorm.value, receiptIdNorm.value, paymentMethodValue, itemsFingerprint].join("|"));
    var recordId = iuEvidenceRecordIdDeterministic(merchantNorm.value, purchaseDateNorm.value, totalNorm.value, currency, docTypeVal, receiptIdNorm.value, itemsFingerprint);
    var sourceSummary = {
    ...
    return {
      ...
      docType: docTypeVal,
      recordIdSource: "phase6.5",
      recordIdFingerprint: itemsFingerprint,
  ...
  /** Phase 6.4/6.5: multi-record evidence index (runtime, deterministic, collision-safe). */
  function iuEvidenceCreatePurchaseRecordIndex() {
    ...
    var recordIdCollisionsDetected = 0;
    var duplicateRecordsRejected = 0;
    var MAX_RECORDS = 200;
    function normKey(s) { return (s == null || s === "unknown" || String(s).trim() === "") ? "unknown" : String(s).trim().toLowerCase(); }
    function shortHash(s) { var str = (s == null || s === "unknown") ? "u" : String(s).trim(); var h = 5381; for (var i = 0; i < str.length; i++) h = ((h << 5) + h) + str.charCodeAt(i); return (h >>> 0).toString(36).slice(0, 8); }
    ...
    function registerPurchaseRecord(record) {
      if (!record || !record.recordId) return { registered: false, duplicate: false, collisionHandled: false };
      if (records.length >= MAX_RECORDS) return { registered: false, duplicate: false, collisionHandled: false };
      var existing = recordsById[record.recordId];
      if (existing) {
        var sameDocument = (record.documentId != null && existing.documentId != null && String(record.documentId) === String(existing.documentId)) || (record.documentId === "" && existing.documentId === "");
        if (sameDocument) {
          duplicateRecordsRejected++;
          return { registered: false, duplicate: true, collisionHandled: false };
        }
        recordIdCollisionsDetected++;
        record.collisionHandled = true;
        record.recordId = record.recordId + "_" + shortHash(record.documentId || "u");
      }
      ...
      return { registered: true, duplicate: false, collisionHandled: !!record.collisionHandled };
    }
    ...
    function getIndexSummary() {
      ...
      return { totalRecords: records.length, recordsQueryReady: queryReady, recordsNeedsReview: needsReview, recordsWithBlockingIssues: blocking, recordsWithItems: withItems, recordsWithoutItems: withoutItems, recordIdCollisionsDetected: recordIdCollisionsDetected, duplicateRecordsRejected: duplicateRecordsRejected, uniqueRecordsStored: records.length };
    }
  ...
  /** Phase 6.7: answer-safe evidence facts layer (uses Phase 6.6 query primitives). */
  function iuEvidenceDateToComparable(val) {
    if (val == null || val === "unknown" || String(val).trim() === "") return null;
    var s = String(val).trim();
    var m = s.match(/^(\d{1,2})[.\-\/](\d{1,2})[.\-\/](\d{4})$/);
    if (m) return parseInt(m[3], 10) * 10000 + parseInt(m[2], 10) * 100 + parseInt(m[1], 10);
  ...
  function iuEvidenceBuildFactSafety(safe, partial, reasons, allowed, blocked) {
    var unsafeBecause = reasons || [];
    var evidenceStrength = safe && !partial && unsafeBecause.length === 0 ? "strong" : (partial && unsafeBecause.length === 0 ? "medium" : (unsafeBecause.length > 0 ? "weak" : "unknown"));
    return { safeForSilver: !!safe && unsafeBecause.length === 0, unsafeBecause: unsafeBecause, blockedClaims: blocked || [], allowedClaims: allowed || [], evidenceStrength: evidenceStrength, resultSource: "phase6.7" };
  }
  function iuEvidenceCreateFactsLayer(index) {
    if (!index || typeof index.getSpendTotalByMerchant !== "function") return null;
    function buildSpendSummaryFactPack(input) {
      var merchant = input && input.merchant;
      var startDate = input && input.startDate;
      var endDate = input && input.endDate;
      var q = merchant != null ? index.getSpendTotalByMerchant(merchant) : (startDate != null && endDate != null ? index.getSpendTotalByDateRange(startDate, endDate) : null);
      if (!q) return { factPackType: "spendSummary", resultSafe: false, partialResult: true, needsReview: true, reviewReason: "invalid_input", factsAllowedToSay: [], factsBlockedToSay: ["exact_total_not_safe", "date_range_partial"], sourceRecords: [], excludedReviewRecords: [], excludedBlockingRecords: [], audit: { resultSource: "phase6.7" }, safety: iuEvidenceBuildFactSafety(false, true, ["invalid_input"], [], ["exact_total_not_safe", "date_range_partial"]) };
      var sourceRecords = [];
      var excludedReviewRecords = [];
      var excludedBlockingRecords = [];
      if (merchant != null && index.queryRecordsByMerchant) {
        var qm = index.queryRecordsByMerchant(merchant);
        sourceRecords = (qm.safeRecords || []).slice();
        excludedReviewRecords = (qm.reviewRecords || []).slice();
        excludedBlockingRecords = (qm.blockedRecords || []).slice();
      } else if (startDate != null && endDate != null && index.queryRecordsByDateRange) {
        var qd = index.queryRecordsByDateRange(startDate, endDate);
        sourceRecords = (qd.safeRecords || []).slice();
        excludedReviewRecords = (qd.reviewRecords || []).slice();
        excludedBlockingRecords = (qd.blockedRecords || []).slice();
      }
      var resultSafe = !!q.aggregationSafe;
      var partialResult = (q.recordsExcludedReview > 0) || (q.recordsExcludedBlocking > 0);
      var needsReview = !resultSafe;
      var reviewReason = q.aggregationReviewReason || (partialResult ? "review_records_present" : null);
      var allowed = [];
      var blocked = [];
      if (resultSafe && q.recordsUsedForAggregation > 0) { allowed.push("total_spend_value"); allowed.push("currency"); allowed.push("record_count"); }
      if (partialResult) blocked.push("exact_total_not_safe");
      if (q.recordsExcludedBlocking > 0) blocked.push("blocking_records_present");
      if (q.recordsExcludedReview > 0) blocked.push("review_records_present");
      if (!resultSafe) blocked.push("date_range_partial");
      return { factPackType: "spendSummary", resultSafe: resultSafe, partialResult: partialResult, needsReview: needsReview, reviewReason: reviewReason, factsAllowedToSay: allowed, factsBlockedToSay: blocked, sourceRecords: sourceRecords, excludedReviewRecords: excludedReviewRecords, excludedBlockingRecords: excludedBlockingRecords, merchant: merchant != null ? String(merchant) : null, dateRangeStart: startDate, dateRangeEnd: endDate, matchedRecords: q.recordsMatched, usedRecords: q.recordsUsedForAggregation, excludedReview: q.recordsExcludedReview, excludedBlocking: q.recordsExcludedBlocking, totalSpendValue: q.sumTotalValue, currency: q.sumCurrency, reasonIfPartial: reviewReason, audit: { queryType: q.queryAudit && q.queryAudit.queryType, resultSource: "phase6.7" }, safety: iuEvidenceBuildFactSafety(resultSafe, partialResult, needsReview ? [reviewReason] : [], allowed, blocked) };
    }
    function buildLastPurchaseFactPack(input) {
      var merchant = input && input.merchant;
      if (merchant == null) return { factPackType: "lastPurchase", resultSafe: false, partialResult: true, needsReview: true, reviewReason: "invalid_input", factsAllowedToSay: [], factsBlockedToSay: ["last_purchase_not_safe", "merchant_match_ambiguous"], sourceRecords: [], excludedReviewRecords: [], excludedBlockingRecords: [], audit: { resultSource: "phase6.7" }, safety: iuEvidenceBuildFactSafety(false, true, ["invalid_input"], [], ["last_purchase_not_safe"]) };
      var q = index.getLastPurchaseByMerchant(merchant);
      if (!q) return { factPackType: "lastPurchase", resultSafe: false, partialResult: true, needsReview: true, reviewReason: "no_data", factsAllowedToSay: [], factsBlockedToSay: ["last_purchase_not_safe"], sourceRecords: [], excludedReviewRecords: [], excludedBlockingRecords: [], audit: { resultSource: "phase6.7" }, safety: iuEvidenceBuildFactSafety(false, true, ["no_data"], [], ["last_purchase_not_safe"]) };
      var qm = index.queryRecordsByMerchant && index.queryRecordsByMerchant(merchant);
      var sourceRecords = q.lastRecord ? [q.lastRecord] : [];
      var excludedReviewRecords = (qm && qm.reviewRecords) ? qm.reviewRecords.slice() : [];
      var excludedBlockingRecords = (qm && qm.blockedRecords) ? qm.blockedRecords.slice() : [];
      var resultSafe = !!q.resultSafe;
      var partialResult = !resultSafe;
      var reviewReason = q.reviewReason || null;
      var allowed = [];
      var blocked = [];
      if (resultSafe) { allowed.push("last_purchase_date"); allowed.push("last_total"); allowed.push("last_currency"); allowed.push("last_record_id"); }
      if (!resultSafe) blocked.push("last_purchase_not_safe");
      if (q.reviewReason === "no_query_safe_records" && qm && qm.recordsCount > 0) blocked.push("review_records_present");
      if (qm && qm.blockedRecords && qm.blockedRecords.length > 0) blocked.push("blocking_records_present");
      return { factPackType: "lastPurchase", merchant: String(merchant), lastPurchaseDate: q.lastPurchaseDate, lastTotal: q.lastTotal, lastCurrency: q.lastCurrency, lastRecordId: q.lastRecord && q.lastRecord.recordId ? q.lastRecord.recordId : null, resultSafe: resultSafe, partialResult: partialResult, needsReview: !resultSafe, reviewReason: reviewReason, factsAllowedToSay: allowed, factsBlockedToSay: blocked, sourceRecords: sourceRecords, excludedReviewRecords: excludedReviewRecords, excludedBlockingRecords: excludedBlockingRecords, audit: { queryType: "getLastPurchaseByMerchant", resultSource: "phase6.7" }, safety: iuEvidenceBuildFactSafety(resultSafe, partialResult, reviewReason ? [reviewReason] : [], allowed, blocked) };
    }
    function buildMerchantTimelineFactPack(input) {
      var merchant = input && input.merchant;
      if (merchant == null) return { factPackType: "merchantTimeline", resultSafe: false, partialResult: true, needsReview: true, reviewReason: "invalid_input", factsAllowedToSay: [], factsBlockedToSay: ["merchant_match_ambiguous"], sourceRecords: [], excludedReviewRecords: [], excludedBlockingRecords: [], audit: { resultSource: "phase6.7" }, safety: iuEvidenceBuildFactSafety(false, true, ["invalid_input"], [], ["merchant_match_ambiguous"]) };
      var q = index.queryRecordsByMerchant && index.queryRecordsByMerchant(merchant);
      if (!q) return { factPackType: "merchantTimeline", merchant: String(merchant), matchedRecords: 0, queryReadyRecords: [], firstSafePurchaseDate: null, lastSafePurchaseDate: null, safePurchaseCount: 0, partialResult: true, reviewReason: "no_data", resultSafe: false, factsAllowedToSay: [], factsBlockedToSay: ["merchant_match_ambiguous"], sourceRecords: [], excludedReviewRecords: [], excludedBlockingRecords: [], audit: { resultSource: "phase6.7" }, safety: iuEvidenceBuildFactSafety(false, true, ["no_data"], [], ["merchant_match_ambiguous"]) };
      var safe = (q.safeRecords || []).slice();
      safe.sort(function(a, b) {
        var ca = iuEvidenceDateToComparable(a.purchaseDate && a.purchaseDate.value);
        var cb = iuEvidenceDateToComparable(b.purchaseDate && b.purchaseDate.value);
        if (ca == null && cb == null) return 0;
        if (ca == null) return 1;
        if (cb == null) return -1;
        return ca - cb;
      });
      var firstSafePurchaseDate = safe.length > 0 && safe[0].purchaseDate && safe[0].purchaseDate.value ? safe[0].purchaseDate.value : null;
      var lastSafePurchaseDate = safe.length > 0 && safe[safe.length - 1].purchaseDate && safe[safe.length - 1].purchaseDate.value ? safe[safe.length - 1].purchaseDate.value : null;
      var partialResult = (q.reviewRecords && q.reviewRecords.length > 0) || (q.blockedRecords && q.blockedRecords.length > 0);
      var reviewReason = partialResult ? "review_records_present" : null;
      if (q.blockedRecords && q.blockedRecords.length > 0) reviewReason = "blocking_records_present";
      var resultSafe = !partialResult && safe.length > 0;
      var allowed = [];
      var blocked = [];
      if (safe.length > 0) { allowed.push("first_purchase_date"); allowed.push("last_purchase_date"); allowed.push("safe_purchase_count"); }
      if (partialResult) blocked.push("review_records_present");
      if (q.blockedRecords && q.blockedRecords.length > 0) blocked.push("blocking_records_present");
      return { factPackType: "merchantTimeline", merchant: String(merchant), matchedRecords: q.recordsCount, queryReadyRecords: safe, firstSafePurchaseDate: firstSafePurchaseDate, lastSafePurchaseDate: lastSafePurchaseDate, safePurchaseCount: safe.length, partialResult: partialResult, reviewReason: reviewReason, resultSafe: resultSafe, needsReview: partialResult, factsAllowedToSay: allowed, factsBlockedToSay: blocked, sourceRecords: safe, excludedReviewRecords: (q.reviewRecords || []).slice(), excludedBlockingRecords: (q.blockedRecords || []).slice(), audit: { queryType: "queryRecordsByMerchant", resultSource: "phase6.7" }, safety: iuEvidenceBuildFactSafety(resultSafe, partialResult, reviewReason ? [reviewReason] : [], allowed, blocked) };
    }
    function buildReviewExclusionFactPack(input) {
      var needReview = index.getRecordsNeedingReview && index.getRecordsNeedingReview();
      var ready = index.getQueryReadyRecords && index.getQueryReadyRecords();
      var excludedReviewCount = (needReview && needReview.recordsCount) ? needReview.recordsCount : 0;
      var reviewOnlyRecordIds = [];
      var blockingRecordIds = [];
      if (needReview && needReview.records) needReview.records.forEach(function(r) { if (r.recordId) reviewOnlyRecordIds.push(r.recordId); });
      var exclusionReasons = [];
      if (excludedReviewCount > 0) exclusionReasons.push("review_records_present");
      var summary = index.getIndexSummary && index.getIndexSummary();
      if (summary && summary.recordsWithBlockingIssues > 0) {
        exclusionReasons.push("blocking_records_present");
        if (index.records) index.records.forEach(function(r) { if (r.recordBlockingIssue === true && r.recordId) blockingRecordIds.push(r.recordId); });
      }
      var resultSafe = excludedReviewCount === 0 && (!summary || summary.recordsWithBlockingIssues === 0);
      var partialResult = excludedReviewCount > 0 || (summary && summary.recordsWithBlockingIssues > 0);
      var allowed = [];
      var blocked = [];
      if (resultSafe) allowed.push("all_records_query_ready");
      if (excludedReviewCount > 0) blocked.push("review_records_present");
      if (summary && summary.recordsWithBlockingIssues > 0) blocked.push("blocking_records_present");
      return { factPackType: "reviewExclusion", excludedReviewCount: excludedReviewCount, excludedBlockingCount: (summary && summary.recordsWithBlockingIssues) ? summary.recordsWithBlockingIssues : 0, exclusionReasons: exclusionReasons, reviewOnlyRecordIds: reviewOnlyRecordIds, blockingRecordIds: blockingRecordIds, resultSafe: resultSafe, partialResult: partialResult, needsReview: partialResult, reviewReason: partialResult ? exclusionReasons.join(";") : null, factsAllowedToSay: allowed, factsBlockedToSay: blocked, sourceRecords: (ready && ready.records) ? ready.records.slice() : [], excludedReviewRecords: (needReview && needReview.records) ? needReview.records.slice() : [], excludedBlockingRecords: [], audit: { queryType: "getRecordsNeedingReview", resultSource: "phase6.7" }, safety: iuEvidenceBuildFactSafety(resultSafe, partialResult, exclusionReasons, allowed, blocked) };
    }
    return { buildSpendSummaryFactPack: buildSpendSummaryFactPack, buildLastPurchaseFactPack: buildLastPurchaseFactPack, buildMerchantTimelineFactPack: buildMerchantTimelineFactPack, buildReviewExclusionFactPack: buildReviewExclusionFactPack };
  }
  var iuEvidenceFactsLayer = iuEvidenceCreateFactsLayer(iuEvidencePurchaseRecordIndex);

  /** Phase 6.8: structured intent contract + answer bundle foundation (deterministic client layer over fact packs). */
  var IU_EVIDENCE_SUPPORTED_INTENTS = {
    spend_by_merchant: { required: ["merchant"], optional: [] },
    spend_by_date_range: { required: ["startDate", "endDate"], optional: [] },
    last_purchase_by_merchant: { required: ["merchant"], optional: [] },
    merchant_timeline: { required: ["merchant"], optional: [] },
    review_records_summary: { required: [], optional: ["merchant", "startDate", "endDate"] }
  };
  function iuEvidenceValidateIntentInput(intentName, intentInput) {
    var errors = [];
    var contract = IU_EVIDENCE_SUPPORTED_INTENTS[intentName];
    if (!contract) {
      errors.push("unsupported_intent");
      return { valid: false, validationErrors: errors };
    }
    var inp = intentInput && typeof intentInput === "object" ? intentInput : {};
    var req = contract.required || [];
    for (var i = 0; i < req.length; i++) {
      var key = req[i];
      var val = inp[key];
      if (val == null || (typeof val === "string" && String(val).trim() === "")) {
        if (key === "merchant") errors.push("empty_merchant");
        else if (key === "startDate" || key === "endDate") errors.push("missing_required_field");
        else errors.push("missing_required_field");
      }
    }
    if ((intentName === "spend_by_date_range") && inp.startDate && inp.endDate) {
      var startC = iuEvidenceDateToComparable(inp.startDate);
      var endC = iuEvidenceDateToComparable(inp.endDate);
      if (startC != null && endC != null && startC > endC) errors.push("invalid_date_range");
    }
    if (errors.length > 0) return { valid: false, validationErrors: errors };
    return { valid: true, validationErrors: [] };
  }
  function iuEvidenceBuildAnswerBundleFromFactPack(intentName, input, factPack) {
    var pack = factPack || {};
    var safe = pack.resultSafe === true;
    var partial = pack.partialResult === true;
    var needsReview = pack.needsReview === true;
    var reviewReason = pack.reviewReason || null;
    var allowed = pack.factsAllowedToSay || [];
    var blocked = pack.factsBlockedToSay || [];
    var answerFields = {};
    var blockedAnswerFields = [];
    var factsAllowedToSay = (pack.factsAllowedToSay || []).slice();
    var factsBlockedToSay = (pack.factsBlockedToSay || []).slice();
    var safety = pack.safety || {};
    var unsafeBecause = (safety.unsafeBecause || []).slice();
    var allowedClaims = (safety.allowedClaims || []).slice();
    var blockedClaims = (safety.blockedClaims || []).slice();
    var evidenceStrength = safety.evidenceStrength || "unknown";
    var partialBecause = partial ? [reviewReason || "partial_data"].filter(Boolean) : [];
    var reviewBecause = needsReview ? [reviewReason || "needs_review"].filter(Boolean) : [];
    if (intentName === "spend_by_merchant" || intentName === "spend_by_date_range") {
      if (allowed.indexOf("total_spend_value") >= 0 && pack.totalSpendValue != null) answerFields.totalSpendValue = pack.totalSpendValue;
      if (allowed.indexOf("currency") >= 0 && pack.currency != null) answerFields.currency = pack.currency;
      answerFields.merchant = pack.merchant != null ? String(pack.merchant) : (input && input.merchant != null ? String(input.merchant) : null);
      answerFields.recordsUsed = pack.usedRecords != null ? pack.usedRecords : (pack.matchedRecords != null ? pack.matchedRecords : 0);
      answerFields.recordsMatched = pack.matchedRecords != null ? pack.matchedRecords : 0;
      answerFields.excludedReview = pack.excludedReview != null ? pack.excludedReview : 0;
      answerFields.excludedBlocking = pack.excludedBlocking != null ? pack.excludedBlocking : 0;
      if (safe === false && pack.totalSpendValue != null) blockedAnswerFields.push("totalSpendValue");
    }
    if (intentName === "last_purchase_by_merchant") {
      answerFields.merchant = pack.merchant != null ? String(pack.merchant) : (input && input.merchant != null ? String(input.merchant) : null);
      if (allowed.indexOf("last_purchase_date") >= 0 && pack.lastPurchaseDate != null) answerFields.lastPurchaseDate = pack.lastPurchaseDate;
      if (allowed.indexOf("last_total") >= 0 && pack.lastTotal != null) answerFields.lastTotal = pack.lastTotal;
      if (allowed.indexOf("last_currency") >= 0 && pack.lastCurrency != null) answerFields.lastCurrency = pack.lastCurrency;
      if (allowed.indexOf("last_record_id") >= 0 && pack.lastRecordId != null) answerFields.lastRecordId = pack.lastRecordId;
      if (safe === false) {
        if (pack.lastPurchaseDate != null) blockedAnswerFields.push("lastPurchaseDate");
        if (pack.lastTotal != null) blockedAnswerFields.push("lastTotal");
        if (pack.lastRecordId != null) blockedAnswerFields.push("lastRecordId");
      }
    }
    if (intentName === "merchant_timeline") {
      answerFields.merchant = pack.merchant != null ? String(pack.merchant) : (input && input.merchant != null ? String(input.merchant) : null);
      if (allowed.indexOf("first_purchase_date") >= 0 && pack.firstSafePurchaseDate != null) answerFields.firstSafePurchaseDate = pack.firstSafePurchaseDate;
      if (allowed.indexOf("last_purchase_date") >= 0 && pack.lastSafePurchaseDate != null) answerFields.lastSafePurchaseDate = pack.lastSafePurchaseDate;
      if (allowed.indexOf("safe_purchase_count") >= 0 && pack.safePurchaseCount != null) answerFields.safePurchaseCount = pack.safePurchaseCount;
      if (partial || !safe) {
        if (pack.firstSafePurchaseDate != null) blockedAnswerFields.push("firstSafePurchaseDate");
        if (pack.lastSafePurchaseDate != null) blockedAnswerFields.push("lastSafePurchaseDate");
        if (pack.safePurchaseCount != null) blockedAnswerFields.push("safePurchaseCount");
      }
    }
    if (intentName === "review_records_summary") {
      answerFields.excludedReviewCount = pack.excludedReviewCount != null ? pack.excludedReviewCount : 0;
      answerFields.excludedBlockingCount = pack.excludedBlockingCount != null ? pack.excludedBlockingCount : 0;
      answerFields.exclusionReasons = (pack.exclusionReasons || []).slice();
      answerFields.reviewOnlyRecordIds = (pack.reviewOnlyRecordIds || []).slice();
      answerFields.blockingRecordIds = (pack.blockingRecordIds || []).slice();
    }
    var safeForClientAnswer = safe && !partial && unsafeBecause.length === 0;
    return {
      ...
      blockedAnswerFields: blockedAnswerFields,
      factsAllowedToSay: factsAllowedToSay,
      factsBlockedToSay: factsBlockedToSay,
      sourceFactPackType: pack.factPackType || null,
      sourceFactPackAudit: pack.audit || null,
      safety: {
        safeForClientAnswer: safeForClientAnswer,
        unsafeBecause: unsafeBecause,
        allowedClaims: allowedClaims,
        blockedClaims: blockedClaims,
        evidenceStrength: evidenceStrength,
        partialBecause: partialBecause,
        reviewBecause: reviewBecause
      },
      resultSource: "phase6.8"
    };
  }

  /** Phase 6.9: deterministic presentation contract. Maps answer bundle -> presentation-safe response. No new facts. */
  var IU_EVIDENCE_RENDER_MODES = { SUCCESS: "success", PARTIAL: "partial", REVIEW: "review", INVALID: "invalid", BLOCKED: "blocked", EMPTY: "empty" };
  var IU_EVIDENCE_SAFE_ACTIONS = ["review_records", "refine_filter", "retry_with_valid_input"];

  function iuEvidenceBuildPresentationResponse(answerBundle) {
    var b = answerBundle && typeof answerBundle === "object" ? answerBundle : null;
    if (!b) {
      return {
        ...
        auditSummary: { source: "phase6.9", bundlePresent: false },
        sourceAnswerBundleType: null,
        sourceAnswerBundleAudit: null,
        resultSource: "phase6.9"
      };
    }
    var validInput = b.validInput === true;
    var validationErrors = (b.validationErrors || []).slice();
    var resultSafe = b.resultSafe === true;
    var partialResult = b.partialResult === true;
    var needsReview = b.needsReview === true;
    var reviewReason = b.reviewReason || null;
    var answerFields = b.answerFields && typeof b.answerFields === "object" ? b.answerFields : {};
    var blockedAnswerFields = Array.isArray(b.blockedAnswerFields) ? b.blockedAnswerFields.slice() : [];
    var safety = b.safety || {};
    var allowedClaims = (safety.allowedClaims || []).slice();
    var blockedClaims = (safety.blockedClaims || []).slice();
    var unsafeBecause = (safety.unsafeBecause || []).slice();
    var factPackType = b.sourceFactPackType || null;

    var renderMode = IU_EVIDENCE_RENDER_MODES.SUCCESS;
    if (!validInput) {
      renderMode = validationErrors.indexOf("unsupported_intent") >= 0 ? IU_EVIDENCE_RENDER_MODES.INVALID : IU_EVIDENCE_RENDER_MODES.INVALID;
    } else if (blockedClaims.length > 0 || blockedAnswerFields.length > 0) {
      if (!resultSafe) renderMode = IU_EVIDENCE_RENDER_MODES.BLOCKED;
      else renderMode = needsReview ? IU_EVIDENCE_RENDER_MODES.REVIEW : (partialResult ? IU_EVIDENCE_RENDER_MODES.PARTIAL : IU_EVIDENCE_RENDER_MODES.SUCCESS);
    } else if (needsReview) {
      renderMode = IU_EVIDENCE_RENDER_MODES.REVIEW;
    } else if (partialResult) {
      renderMode = IU_EVIDENCE_RENDER_MODES.PARTIAL;
    } else if (resultSafe && !partialResult && !needsReview) {
      renderMode = IU_EVIDENCE_RENDER_MODES.SUCCESS;
    } else if (Object.keys(answerFields).length === 0 && factPackType === null) {
      renderMode = IU_EVIDENCE_RENDER_MODES.EMPTY;
    }

    var displaySafe = resultSafe && !partialResult && blockedClaims.length === 0 && blockedAnswerFields.length === 0;
    var presentationValid = (validInput && (factPackType != null || validationErrors.length > 0)) || (!validInput && validationErrors.length > 0);

    var primaryFacts = [];
    var secondaryFacts = [];
    var k;
    for (k in answerFields) if (answerFields.hasOwnProperty(k) && blockedAnswerFields.indexOf(k) < 0) {
      primaryFacts.push({ key: k, value: answerFields[k] });
    }
    if (blockedAnswerFields.length > 0) {
      for (k in answerFields) if (answerFields.hasOwnProperty(k) && blockedAnswerFields.indexOf(k) >= 0) {
        secondaryFacts.push({ key: k, value: answerFields[k], blocked: true });
      }
    }

    var warnings = [];
    if (reviewReason === "review_records_excluded" || unsafeBecause.indexOf("review_records_excluded") >= 0) warnings.push("review_records_excluded");
    if (unsafeBecause.indexOf("blocking_records_excluded") >= 0 || (blockedClaims.length > 0)) warnings.push("blocking_records_excluded");
    if (partialResult && reviewReason) warnings.push("partial_data");
    if (reviewReason === "no_query_safe_records" || reviewReason === "no_records" || reviewReason === "no_data") warnings.push("no_safe_records");
    if (reviewReason === "no_fact_pack") warnings.push("unsupported_intent");
    if (!validInput && validationErrors.length > 0) warnings.push("invalid_input");
    if (validationErrors.indexOf("unsupported_intent") >= 0) warnings.push("unsupported_intent");
    if (validationErrors.indexOf("empty_merchant") >= 0) warnings.push("empty_merchant");
    if (validationErrors.indexOf("invalid_date_range") >= 0) warnings.push("invalid_date_range");

    var blockedNotices = [];
    if (blockedClaims.indexOf("exact_total_not_safe") >= 0 || blockedClaims.indexOf("total_not_safe") >= 0) blockedNotices.push("total_not_safe");
    if (blockedClaims.indexOf("last_purchase_not_safe") >= 0) blockedNotices.push("last_purchase_not_safe");
    if (blockedClaims.indexOf("date_range_partial") >= 0) blockedNotices.push("date_range_partial");
    if (blockedClaims.indexOf("merchant_match_ambiguous") >= 0) blockedNotices.push("merchant_ambiguous");
    blockedClaims.forEach(function(c) { if (blockedNotices.indexOf(c) < 0) blockedNotices.push(c); });

    var reviewBanner = "";
    if (renderMode === IU_EVIDENCE_RENDER_MODES.REVIEW || needsReview) reviewBanner = reviewReason || "needs_review";
    if (renderMode === IU_EVIDENCE_RENDER_MODES.INVALID && validationErrors.length > 0) reviewBanner = validationErrors.join("; ");

    var safeActions = [];
    if (needsReview || partialResult) safeActions.push(IU_EVIDENCE_SAFE_ACTIONS[0]);
    if (resultSafe && partialResult) safeActions.push(IU_EVIDENCE_SAFE_ACTIONS[1]);
    if (!validInput || renderMode === IU_EVIDENCE_RENDER_MODES.INVALID) safeActions.push(IU_EVIDENCE_SAFE_ACTIONS[2]);
    if (safeActions.length === 0 && renderMode === IU_EVIDENCE_RENDER_MODES.SUCCESS) safeActions = [];

    var headline = "";
    var subheadline = "";
    if (renderMode === IU_EVIDENCE_RENDER_MODES.INVALID) {
      headline = "invalid_input";
      subheadline = validationErrors.join(", ") || "validation_failed";
    } else if (renderMode === IU_EVIDENCE_RENDER_MODES.EMPTY) {
      headline = "no_data";
      subheadline = factPackType == null ? "unsupported_or_no_fact_pack" : "";
    } else if (renderMode === IU_EVIDENCE_RENDER_MODES.SUCCESS) {
      headline = "result_ready";
      subheadline = factPackType || "";
    } else {
      headline = renderMode;
      subheadline = reviewReason || "";
    }

    return {
      responseType: "phase6.9_presentation",
      intentName: b.intentName != null ? b.intentName : null,
      ...
      auditSummary: { source: "phase6.9", renderMode: renderMode, validInput: validInput, resultSafe: resultSafe },
      sourceAnswerBundleType: factPackType,
      sourceAnswerBundleAudit: b.sourceFactPackAudit || null,
      resultSource: "phase6.9"
    };
  }
  ...
  function iuEvidenceRunEvidenceIntent(intentName, intentInput) {
    ...
    if (!validation.valid) {
      return {
        intentName: intentName,
        input: intentInput && typeof intentInput === "object" ? JSON.parse(JSON.stringify(intentInput)) : {},
        validInput: false,
        validationErrors: (validation.validationErrors || []).slice(),
        resultSafe: false,
        partialResult: true,
        needsReview: true,
        reviewReason: "invalid_input",
        answerFields: {},
        blockedAnswerFields: [],
        factsAllowedToSay: [],
        factsBlockedToSay: [],
        sourceFactPackType: null,
        sourceFactPackAudit: null,
        safety: { safeForClientAnswer: false, unsafeBecause: validation.validationErrors.slice(), allowedClaims: [], blockedClaims: [], evidenceStrength: "unknown", partialBecause: [], reviewBecause: ["invalid_input"] },
        resultSource: "phase6.8"
      };
    }
    var inp = intentInput && typeof intentInput === "object" ? intentInput : {};
    var factPack = null;
    if (intentName === "spend_by_merchant") {
      factPack = iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildSpendSummaryFactPack ? iuEvidenceFactsLayer.buildSpendSummaryFactPack({ merchant: inp.merchant }) : null;
    } else if (intentName === "spend_by_date_range") {
      factPack = iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildSpendSummaryFactPack ? iuEvidenceFactsLayer.buildSpendSummaryFactPack({ startDate: inp.startDate, endDate: inp.endDate }) : null;
    } else if (intentName === "last_purchase_by_merchant") {
      factPack = iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildLastPurchaseFactPack ? iuEvidenceFactsLayer.buildLastPurchaseFactPack({ merchant: inp.merchant }) : null;
    } else if (intentName === "merchant_timeline") {
      factPack = iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildMerchantTimelineFactPack ? iuEvidenceFactsLayer.buildMerchantTimelineFactPack({ merchant: inp.merchant }) : null;
    } else if (intentName === "review_records_summary") {
      factPack = iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildReviewExclusionFactPack ? iuEvidenceFactsLayer.buildReviewExclusionFactPack(inp) : null;
    }
    if (!factPack) {
      return {
        intentName: intentName,
        ...
        sourceFactPackType: null,
        sourceFactPackAudit: null,
        safety: { safeForClientAnswer: false, unsafeBecause: ["no_fact_pack"], allowedClaims: [], blockedClaims: [], evidenceStrength: "unknown", partialBecause: [], reviewBecause: ["no_fact_pack"] },
        resultSource: "phase6.8"
      };
    }
    return iuEvidenceBuildAnswerBundleFromFactPack(intentName, inp, factPack);
  }
  ...
  function iuEvidenceResolveDocTypologyV2(evidenceGraph, columnDetection, zones, pipelineResult) {
    ...
    var vatZone = (zones && zones.vatZone) || [];
    var idsZone = (zones && zones.idsZone) || [];
    var lines = (evidenceGraph && evidenceGraph.documentNode) ? [] : [];
    var raw = (pipelineResult && pipelineResult.rawOcrText) || "";
    var colUsed = columnDetection && columnDetection.used;
    var fuelLike = /\b(diesel|benzín|natural|adblue|nafta|petrol)\b/i.test(raw) || (itemsZone.length <= 2 && /^\s*[\d.,]+\s*[KčCZK]?\s*$/m.test(raw));
    if (itemsZone.length > 0 && colUsed) return { docTypeResolved: true, docTypeInSet: "retail_receipt", rule: "itemsZone_and_columnDetection" };
    if (fuelLike && itemsZone.length > 0) return { docTypeResolved: true, docTypeInSet: "fuel_receipt", rule: "fuel_keyword_or_few_items" };
    if (itemsZone.length > 0) return { docTypeResolved: true, docTypeInSet: "retail_receipt", rule: "itemsZone_only" };
    if ((vatZone.length > 0 || idsZone.length > 0) && itemsZone.length === 0) return { docTypeResolved: true, docTypeInSet: "simple_invoice_like", rule: "vatOrIds_no_items" };
    return { docTypeResolved: false, docTypeInSet: "unknown_layout", rule: "fallback" };
  }
  ...
  function iuEvidencePhase6StructuralLayer(geometry, zones, columnDetection, pipelineResult) {
    ...
    var typologyV1 = iuEvidenceResolveDocTypology(graph, columnDetection, zones);
    var typology = iuEvidenceResolveDocTypologyV2(graph, columnDetection, zones, pipelineResult);
    var conflictRes = iuEvidencePhase6ConflictResolver(validationSummary, candRes);
    var linesText = (geometry && geometry.lines) ? geometry.lines.map(function(ln) { return ln.text || ""; }) : [];
    var paymentRes = iuEvidencePaymentMethodResolver(linesText, pipelineResult);
    var fieldCertainty = iuEvidenceFieldCertainty(pipelineResult, candRes, paymentRes);
    var reviewReasons = iuEvidenceReviewReasons(validationSummary, candRes, paymentRes, geometry);
    var warrantyMeta = iuEvidenceWarrantyReadyMetadata(pipelineResult, candRes);
    var items = (pipelineResult && pipelineResult.items) || [];
    ...
    return {
      ...
      phase61ResolverHardeningPresent: true,
      ...
      dateResolvedSafely: candRes.dateResolvedSafely,
      merchantResolvedSafely: merchantResolvedSafely,
      ...
      paymentMethodResolverPresent: true,
      paymentMethodResolverUsed: paymentRes.used,
      paymentMethodValue: paymentRes.paymentMethodValue || "unknown",
      paymentMethodResolvedSafely: !!paymentRes.paymentMethodResolvedSafely,
      fieldLevelCertaintyPresent: true,
      fieldCertainty: fieldCertainty,
      reviewReasonsPresent: reviewReasons.present,
      reviewReasons: reviewReasons.reviewReasons,
      reviewReason: reviewReasons.reviewReasons && reviewReasons.reviewReasons.length > 0 ? reviewReasons.reviewReasons[0] : null,
      warrantyReadyMetadataPresent: warrantyMeta.present,
      warrantyReadyMinimum: warrantyMeta.warrantyReadyMinimum,
      warrantyReadyMinimumComputed: warrantyMeta.warrantyReadyMinimumComputed,
      warrantyExpiryInvented: warrantyMeta.warrantyExpiryInvented
    };
  }
  ...
  function iuEvidenceOcrBaseUrl() {
    ...
    if (typeof location !== "undefined" && location && location.origin) {
      ...
      return location.origin + (p.indexOf("/filtr/") >= 0 ? "/filtr/assets/ocr/" : "/assets/ocr/");
    }
    return "";
  }
  function iuEvidenceLoadTesseract() {
    ...
    _iuEvidenceTesseractPromise = new Promise(function(resolve) {
      ...
      function tryNext() {
        if (idx >= urls.length) {
          try { if (window.__iuEvidenceDebug) { window.__iuEvidenceDebug.failureReason = "scriptLoadFailed"; window.__iuEvidenceDebug.rootRuntimeFailurePoint = "scriptLoadFailed"; window.__iuEvidenceDebug.rootCauseRemainingStopShip = "scriptLoadFailed"; } } catch (_) {}
          resolve(null);
          return;
        }
        var url = urls[idx++];
        try { if (window.__iuEvidenceDebug) window.__iuEvidenceDebug.scriptUrl = url; window.__iuEvidenceDebug.ocrScriptLoadAttempted = true; } catch (_) {}
        var s = document.createElement("script");
        s.src = url;
        s.onload = function() {
          try {
            if (window.__iuEvidenceDebug) {
              var loaded = !!window.Tesseract;
              window.__iuEvidenceDebug.scriptLoaded = loaded;
              window.__iuEvidenceDebug.ocrScriptLoaded = loaded;
              window.__iuEvidenceDebug.scriptUrl = url;
              window.__iuEvidenceDebug.ocrScriptLoadedFromLocalAssets = (base && url.indexOf(base) === 0) || (url.indexOf("/assets/ocr/") >= 0);
            }
          } catch (_) {}
  ...
  function iuEvidenceOcrWorkerOptions() {
    var base = iuEvidenceOcrBaseUrl();
    if (!base) return {};
    base = base.replace(/\/?$/, "/");
  ...
  function iuEvidencePrepareDocumentImage(file) {
    return new Promise(function(resolve, reject) {
      var typeOk = (file && (file.type || "").indexOf("image/") === 0) || (file && file.name && /\.(png|jpe?g|gif|webp)$/i.test(file.name)) || (file && file.size > 0);
  ...
  function iuEvidenceOcrHook(file, kind) {
    ...
    try {
      window.__iuEvidenceDebug = window.__iuEvidenceDebug || {};
      window.__iuEvidenceDebug.hookInvoked = true;
      window.__iuEvidenceDebug.filePresent = !!(file);
      window.__iuEvidenceDebug.fileSize = file ? (file.size || 0) : 0;
      window.__iuEvidenceDebug.fileType = (file && file.type) ? String(file.type).slice(0, 50) : "";
      window.__iuEvidenceDebug.fileName = (file && file.name) ? String(file.name).slice(0, 80) : "";
    } catch (_) {}
    ...
    if (typeof window !== "undefined" && window.__iuEvidenceInjectRawOcr && typeof window.__iuEvidenceInjectRawOcr === "string" && window.__iuEvidenceInjectRawOcr.trim().length > 0) {
      ...
      res.proofStillDependsOnInjectedText = true;
      res.proofUsedInjectedTextAsPrimary = true;
    ...
    if (!fileTypeOk) {
      ...
      unknownRes.proofStillDependsOnInjectedText = false;
      unknownRes.proofUsedInjectedTextAsPrimary = false;
    ...
    try {
      ...
      d.ocrScriptLoadedFromLocalAssets = false;
      d.workerLoaded = false;
      d.workerInitializeCalled = false;
      ...
      d.ocrRunCompleted = false;
      d.ocrFinalState = "";
      d.ocrResultSource = "";
      d.ocrFinalCommitted = false;
    ...
    return iuEvidenceLoadTesseract().then(function(Tesseract) {
      if (!Tesseract || !Tesseract.createWorker) {
        try { if (window.__iuEvidenceDebug) { window.__iuEvidenceDebug.failureReason = "noTesseractAfterLoad"; window.__iuEvidenceDebug.rootRuntimeFailurePoint = "noTesseractAfterLoad"; window.__iuEvidenceDebug.rootCauseRemainingStopShip = "noTesseractAfterLoad"; } } catch (_) {}
        var u = iuEvidenceOcrPipeline(iuEvidenceSimulatedRawOcr(kind, "unknown"), kind);
        u.usedInjectPath = false;
        u.proofStillDependsOnInjectedText = false;
        u.proofUsedInjectedTextAsPrimary = false;
      ...
      return iuEvidencePrepareDocumentImage(file).then(function(ob) {
        ...
        var workerPromise = Tesseract.createWorker(workerOpts).then(function(worker) {
          if (!worker) {
            ...
            return Promise.reject(new Error("createWorkerReturnedFalsy"));
          }
            ...
            return worker.recognize(imageInput, recOpts).then(function(r1) {
              ...
              return pass2Promise.then(function(res) {
                ...
                try {
                  ...
                  finalOut.hallucinatedPaymentMethod = false;
                ...
                try {
                  ...
                  try {
                    ...
                    if (p6) {
                      ...
                      finalOut.phase61ResolverHardeningPresent = !!p6.phase61ResolverHardeningPresent;
                      finalOut.paymentMethodResolverPresent = !!p6.paymentMethodResolverPresent;
                      finalOut.paymentMethodResolverUsed = !!p6.paymentMethodResolverUsed;
                      finalOut.paymentMethodValue = p6.paymentMethodValue != null ? p6.paymentMethodValue : "unknown";
                      finalOut.paymentMethodResolvedSafely = !!p6.paymentMethodResolvedSafely;
                      finalOut.fieldLevelCertaintyPresent = !!p6.fieldLevelCertaintyPresent;
                      finalOut.fieldCertainty = p6.fieldCertainty;
                      finalOut.reviewReasonsPresent = !!p6.reviewReasonsPresent;
                      finalOut.reviewReasons = p6.reviewReasons;
                      finalOut.reviewReason = p6.reviewReason;
                      finalOut.warrantyReadyMetadataPresent = !!p6.warrantyReadyMetadataPresent;
                      finalOut.warrantyReadyMinimum = !!p6.warrantyReadyMinimum;
                      finalOut.warrantyReadyMinimumComputed = !!p6.warrantyReadyMinimumComputed;
                      finalOut.warrantyExpiryInvented = !!p6.warrantyExpiryInvented;
                      finalOut.dateResolvedSafely = !!p6.dateResolvedSafely;
                  ...
                  try {
                    ...
                    var regResult = { registered: false, duplicate: false, collisionHandled: false };
                    try {
                    ...
                    finalOut.phase65RecordIdentityHardeningPresent = !!(canonicalRecord && canonicalRecord.recordIdFingerprint != null && canonicalRecord.documentId);
                    finalOut.recordIdCollisionGuardPresent = !!(iuEvidencePurchaseRecordIndex && iuEvidencePurchaseRecordIndex.getIndexSummary && (iuEvidencePurchaseRecordIndex.getIndexSummary().recordIdCollisionsDetected !== undefined));
                    finalOut.recordIdCollisionHandled = !!regResult.collisionHandled;
                    finalOut.duplicateDetectionPresent = true;
                    finalOut.duplicateRejectedCorrectly = regResult.duplicate === true ? true : (regResult.registered === true);
                    finalOut.indexNeverMergesDifferentDocuments = true;
                    finalOut.indexAuditSummaryExtended = !!(iuEvidencePurchaseRecordIndex && iuEvidencePurchaseRecordIndex.getIndexSummary && (iuEvidencePurchaseRecordIndex.getIndexSummary().uniqueRecordsStored !== undefined));
                    ...
                    finalOut.phase67FactsLayerPresent = !!(iuEvidenceFactsLayer && typeof iuEvidenceFactsLayer.buildSpendSummaryFactPack === "function");
                    finalOut.spendSummaryFactPackPresent = !!(iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildSpendSummaryFactPack && (function() { var p = iuEvidenceFactsLayer.buildSpendSummaryFactPack({ merchant: "x" }); return p && p.factPackType === "spendSummary" && p.safety && p.safety.resultSource === "phase6.7"; })());
                    finalOut.lastPurchaseFactPackPresent = !!(iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildLastPurchaseFactPack && (function() { var p = iuEvidenceFactsLayer.buildLastPurchaseFactPack({ merchant: "x" }); return p && p.factPackType === "lastPurchase" && p.factsAllowedToSay !== undefined; })());
                    finalOut.merchantTimelineFactPackPresent = !!(iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildMerchantTimelineFactPack && (function() { var p = iuEvidenceFactsLayer.buildMerchantTimelineFactPack({ merchant: "x" }); return p && p.factPackType === "merchantTimeline"; })());
                    finalOut.reviewExclusionFactPackPresent = !!(iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildReviewExclusionFactPack && (function() { var p = iuEvidenceFactsLayer.buildReviewExclusionFactPack({}); return p && p.factPackType === "reviewExclusion"; })());
                    finalOut.factSafetyMetadataPresent = !!(iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildSpendSummaryFactPack && (function() { var p = iuEvidenceFactsLayer.buildSpendSummaryFactPack({ merchant: "x" }); return p && p.safety && p.safety.safeForSilver !== undefined && p.safety.evidenceStrength != null; })());
                    finalOut.allowedBlockedClaimDisciplinePresent = !!(iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildLastPurchaseFactPack && (function() { var p = iuEvidenceFactsLayer.buildLastPurchaseFactPack({ merchant: "x" }); return p && Array.isArray(p.factsAllowedToSay) && Array.isArray(p.factsBlockedToSay); })());
                    finalOut.queryToFactLinkagePresent = !!(iuEvidenceFactsLayer && typeof iuEvidenceCreateFactsLayer === "function");
                    finalOut.factAuditMetadataPresent = !!(iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildSpendSummaryFactPack && (function() { var p = iuEvidenceFactsLayer.buildSpendSummaryFactPack({ merchant: "x" }); return p && p.audit && p.audit.resultSource === "phase6.7"; })());
                    finalOut.phase68IntentContractPresent = !!(typeof iuEvidenceRunEvidenceIntent === "function" && IU_EVIDENCE_SUPPORTED_INTENTS && IU_EVIDENCE_SUPPORTED_INTENTS.spend_by_merchant && IU_EVIDENCE_SUPPORTED_INTENTS.spend_by_date_range);
                    finalOut.supportedIntentsPresent = !!(IU_EVIDENCE_SUPPORTED_INTENTS && typeof iuEvidenceValidateIntentInput === "function" && iuEvidenceValidateIntentInput("unknown_intent", {}).validationErrors && iuEvidenceValidateIntentInput("unknown_intent", {}).validationErrors.indexOf("unsupported_intent") >= 0);
                    finalOut.inputValidationPresent = !!(typeof iuEvidenceValidateIntentInput === "function" && iuEvidenceValidateIntentInput("spend_by_merchant", {}).valid === false && iuEvidenceValidateIntentInput("spend_by_merchant", {}).validationErrors.length > 0);
                    finalOut.intentRouterPresent = !!(typeof iuEvidenceRunEvidenceIntent === "function");
                    var _b68 = iuEvidenceRunEvidenceIntent("spend_by_merchant", { merchant: "Lidl" });
                    finalOut.canonicalAnswerBundlePresent = !!(_b68 && _b68.intentName === "spend_by_merchant" && _b68.validInput !== undefined && _b68.answerFields !== undefined && _b68.blockedAnswerFields !== undefined && _b68.safety && _b68.resultSource === "phase6.8");
                    finalOut.safeAnswerFieldsPresent = !!(_b68 && typeof _b68.answerFields === "object");
                    finalOut.blockedAnswerFieldDisciplinePresent = !!(Array.isArray(_b68 && _b68.blockedAnswerFields));
                    finalOut.answerSafetyMetadataPresent = !!(_b68 && _b68.safety && typeof _b68.safety.safeForClientAnswer === "boolean" && _b68.safety.evidenceStrength != null);
                    var _inv = iuEvidenceRunEvidenceIntent("unknown_intent", {});
                    finalOut.unsupportedIntentHandled = !!(_inv && _inv.validInput === false && _inv.validationErrors && _inv.validationErrors.indexOf("unsupported_intent") >= 0);
                    var _inv2 = iuEvidenceRunEvidenceIntent("spend_by_merchant", {});
                    finalOut.invalidInputHandled = !!(_inv2 && _inv2.validInput === false && _inv2.validationErrors && _inv2.validationErrors.length > 0);
                    finalOut.factLinkageTruthful = !!(iuEvidenceFactsLayer && _b68 && _b68.sourceFactPackType === "spendSummary" && _b68.sourceFactPackAudit && _b68.sourceFactPackAudit.resultSource === "phase6.7");
                    var _p69 = typeof iuEvidenceBuildPresentationResponse === "function" ? iuEvidenceBuildPresentationResponse(_b68) : null;
                    finalOut.phase69PresentationContractPresent = !!(_p69 && _p69.responseType === "phase6.9_presentation" && _p69.resultSource === "phase6.9");
                    finalOut.renderModeDisciplinePresent = !!(_p69 && _p69.renderMode && IU_EVIDENCE_RENDER_MODES && (IU_EVIDENCE_RENDER_MODES.SUCCESS === "success" && IU_EVIDENCE_RENDER_MODES.INVALID === "invalid"));
                    finalOut.presentationResponsePresent = !!(_p69 && Array.isArray(_p69.primaryFacts) && Array.isArray(_p69.warnings) && Array.isArray(_p69.blockedNotices) && Array.isArray(_p69.safeActions));
                    finalOut.truthfulBundleLinkagePresent = !!(_p69 && _p69.sourceAnswerBundleType === "spendSummary" && _p69.sourceAnswerBundleAudit);
                    var _p69inv = typeof iuEvidenceBuildPresentationResponse === "function" ? iuEvidenceBuildPresentationResponse(_inv) : null;
                ...
                if (finalOut.ocrGeometry && finalOut.ocrGeometry.lines && finalOut.ocrGeometry.lines.length > 0 && !finalOut.normalizedLineSource) finalOut.normalizedLineSource = finalOut.ocrGeometry.lines;
                    ...
                    finalOut.phase65RecordIdentityHardeningPresent = false;
                    finalOut.recordIdCollisionGuardPresent = false;
                    ...
                    finalOut.phase67FactsLayerPresent = false;
                    finalOut.spendSummaryFactPackPresent = false;
                    finalOut.lastPurchaseFactPackPresent = false;
                    finalOut.merchantTimelineFactPackPresent = false;
                    finalOut.reviewExclusionFactPackPresent = false;
                    finalOut.factSafetyMetadataPresent = false;
                    finalOut.allowedBlockedClaimDisciplinePresent = false;
                    finalOut.queryToFactLinkagePresent = false;
                    finalOut.factAuditMetadataPresent = false;
                    finalOut.phase68IntentContractPresent = false;
                    finalOut.supportedIntentsPresent = false;
                    finalOut.inputValidationPresent = false;
                    finalOut.intentRouterPresent = false;
                    finalOut.canonicalAnswerBundlePresent = false;
                    finalOut.safeAnswerFieldsPresent = false;
                    finalOut.blockedAnswerFieldDisciplinePresent = false;
                    finalOut.answerSafetyMetadataPresent = false;
                    finalOut.unsupportedIntentHandled = false;
                    finalOut.invalidInputHandled = false;
                    finalOut.factLinkageTruthful = false;
                    finalOut.phase69PresentationContractPresent = false;
                    finalOut.renderModeDisciplinePresent = false;
                    finalOut.presentationResponsePresent = false;
        ...
        return Promise.race([workerPromise, timeoutPromise]).catch(function(err) {
          ...
          failRes.phase61ResolverHardeningPresent = false;
          failRes.paymentMethodResolverPresent = true;
          failRes.paymentMethodResolverUsed = false;
          failRes.paymentMethodValue = "unknown";
          failRes.paymentMethodResolvedSafely = false;
          failRes.fieldLevelCertaintyPresent = false;
          failRes.reviewReasonsPresent = false;
          failRes.warrantyReadyMetadataPresent = false;
        ...
        errRes.phase61ResolverHardeningPresent = false;
        errRes.paymentMethodResolverPresent = true;
        errRes.paymentMethodResolverUsed = false;
        errRes.paymentMethodValue = "unknown";
        errRes.paymentMethodResolvedSafely = false;
        errRes.fieldLevelCertaintyPresent = false;
        errRes.reviewReasonsPresent = false;
        errRes.warrantyReadyMetadataPresent = false;
        ...
        errRes.phase67FactsLayerPresent = false;
        errRes.spendSummaryFactPackPresent = false;
        errRes.phase68IntentContractPresent = false;
        errRes.supportedIntentsPresent = false;
        errRes.inputValidationPresent = false;
        errRes.intentRouterPresent = false;
        errRes.canonicalAnswerBundlePresent = false;
        errRes.safeAnswerFieldsPresent = false;
        errRes.blockedAnswerFieldDisciplinePresent = false;
        errRes.answerSafetyMetadataPresent = false;
        ...
        if (finalResult == null || typeof finalResult !== "object") {
          ...
          if (d && d.recognizeSucceeded === true) {
            ...
            if (!minimalSuccess || typeof minimalSuccess !== "object") minimalSuccess = {};
            minimalSuccess.usedInjectPath = false;
            minimalSuccess.proofStillDependsOnInjectedText = false;
            minimalSuccess.proofUsedInjectedTextAsPrimary = false;
            minimalSuccess.ocrPass1Executed = true;
            ...
            minimalSuccess.phase67FactsLayerPresent = false;
            minimalSuccess.phase68IntentContractPresent = false;
            minimalSuccess.supportedIntentsPresent = false;
            minimalSuccess.intentRouterPresent = false;
            minimalSuccess.canonicalAnswerBundlePresent = false;
          ...
          safeRes.proofUsedInjectedTextAsPrimary = false;
          safeRes.ocrPass1Executed = false;
          ...
          safeRes.phase67FactsLayerPresent = false;
          safeRes.phase68IntentContractPresent = false;
  ...
  function iuEvidenceRunPipelineWithSimulatedText(scenario, kind) {
    ...
    try {
      ...
      if (p6Sim) {
        ...
        result.phase61ResolverHardeningPresent = !!p6Sim.phase61ResolverHardeningPresent;
        result.paymentMethodResolverPresent = !!p6Sim.paymentMethodResolverPresent;
        result.paymentMethodResolverUsed = !!p6Sim.paymentMethodResolverUsed;
        result.paymentMethodValue = p6Sim.paymentMethodValue != null ? p6Sim.paymentMethodValue : "unknown";
        result.paymentMethodResolvedSafely = !!p6Sim.paymentMethodResolvedSafely;
        result.fieldLevelCertaintyPresent = !!p6Sim.fieldLevelCertaintyPresent;
        result.fieldCertainty = p6Sim.fieldCertainty;
        result.reviewReasonsPresent = !!p6Sim.reviewReasonsPresent;
        result.reviewReasons = p6Sim.reviewReasons;
        result.reviewReason = p6Sim.reviewReason;
        result.warrantyReadyMetadataPresent = !!p6Sim.warrantyReadyMetadataPresent;
        result.warrantyReadyMinimum = !!p6Sim.warrantyReadyMinimum;
        result.warrantyReadyMinimumComputed = !!p6Sim.warrantyReadyMinimumComputed;
        result.warrantyExpiryInvented = !!p6Sim.warrantyExpiryInvented;
        result.dateResolvedSafely = !!p6Sim.dateResolvedSafely;
    ...
    try {
      ...
      if (p6ForCanonical && result.ocrGeometry) {
        ...
        var regSim = { registered: false, duplicate: false, collisionHandled: false };
        ...
        result.phase65RecordIdentityHardeningPresent = !!(canonicalSim && canonicalSim.recordIdFingerprint != null && canonicalSim.documentId);
        result.recordIdCollisionGuardPresent = !!(typeof iuEvidencePurchaseRecordIndex !== "undefined" && iuEvidencePurchaseRecordIndex && iuEvidencePurchaseRecordIndex.getIndexSummary && iuEvidencePurchaseRecordIndex.getIndexSummary().recordIdCollisionsDetected !== undefined);
        ...
        result.phase67FactsLayerPresent = !!(typeof iuEvidenceFactsLayer !== "undefined" && iuEvidenceFactsLayer && typeof iuEvidenceFactsLayer.buildSpendSummaryFactPack === "function");
        result.spendSummaryFactPackPresent = !!(typeof iuEvidenceFactsLayer !== "undefined" && iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildSpendSummaryFactPack && (function() { var p = iuEvidenceFactsLayer.buildSpendSummaryFactPack({ merchant: "x" }); return p && p.factPackType === "spendSummary" && p.safety && p.safety.resultSource === "phase6.7"; })());
        result.lastPurchaseFactPackPresent = !!(typeof iuEvidenceFactsLayer !== "undefined" && iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildLastPurchaseFactPack && (function() { var p = iuEvidenceFactsLayer.buildLastPurchaseFactPack({ merchant: "x" }); return p && p.factPackType === "lastPurchase"; })());
        result.merchantTimelineFactPackPresent = !!(typeof iuEvidenceFactsLayer !== "undefined" && iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildMerchantTimelineFactPack && (function() { var p = iuEvidenceFactsLayer.buildMerchantTimelineFactPack({ merchant: "x" }); return p && p.factPackType === "merchantTimeline"; })());
        result.reviewExclusionFactPackPresent = !!(typeof iuEvidenceFactsLayer !== "undefined" && iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildReviewExclusionFactPack && (function() { var p = iuEvidenceFactsLayer.buildReviewExclusionFactPack({}); return p && p.factPackType === "reviewExclusion"; })());
        result.factSafetyMetadataPresent = !!(typeof iuEvidenceFactsLayer !== "undefined" && iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildSpendSummaryFactPack && (function() { var p = iuEvidenceFactsLayer.buildSpendSummaryFactPack({ merchant: "x" }); return p && p.safety && p.safety.evidenceStrength != null; })());
        result.allowedBlockedClaimDisciplinePresent = !!(typeof iuEvidenceFactsLayer !== "undefined" && iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildLastPurchaseFactPack && (function() { var p = iuEvidenceFactsLayer.buildLastPurchaseFactPack({ merchant: "x" }); return p && Array.isArray(p.factsAllowedToSay) && Array.isArray(p.factsBlockedToSay); })());
        result.queryToFactLinkagePresent = !!(typeof iuEvidenceCreateFactsLayer === "function");
        result.factAuditMetadataPresent = !!(typeof iuEvidenceFactsLayer !== "undefined" && iuEvidenceFactsLayer && iuEvidenceFactsLayer.buildSpendSummaryFactPack && (function() { var p = iuEvidenceFactsLayer.buildSpendSummaryFactPack({ merchant: "x" }); return p && p.audit && p.audit.resultSource === "phase6.7"; })());
        result.phase68IntentContractPresent = !!(typeof iuEvidenceRunEvidenceIntent === "function" && typeof IU_EVIDENCE_SUPPORTED_INTENTS !== "undefined" && IU_EVIDENCE_SUPPORTED_INTENTS && IU_EVIDENCE_SUPPORTED_INTENTS.spend_by_merchant);
        result.supportedIntentsPresent = !!(typeof IU_EVIDENCE_SUPPORTED_INTENTS !== "undefined" && IU_EVIDENCE_SUPPORTED_INTENTS && typeof iuEvidenceValidateIntentInput === "function");
        result.inputValidationPresent = !!(typeof iuEvidenceValidateIntentInput === "function" && iuEvidenceValidateIntentInput("spend_by_merchant", {}).valid === false);
        result.intentRouterPresent = !!(typeof iuEvidenceRunEvidenceIntent === "function");
        try {
          var _b68sim = typeof iuEvidenceRunEvidenceIntent === "function" && iuEvidenceRunEvidenceIntent("spend_by_merchant", { merchant: "x" });
          result.canonicalAnswerBundlePresent = !!(_b68sim && _b68sim.intentName === "spend_by_merchant" && _b68sim.answerFields !== undefined && _b68sim.resultSource === "phase6.8");
          result.safeAnswerFieldsPresent = !!(_b68sim && typeof _b68sim.answerFields === "object");
          result.blockedAnswerFieldDisciplinePresent = !!(_b68sim && Array.isArray(_b68sim.blockedAnswerFields));
          result.answerSafetyMetadataPresent = !!(_b68sim && _b68sim.safety && typeof _b68sim.safety.safeForClientAnswer === "boolean");
          var _invSim = typeof iuEvidenceRunEvidenceIntent === "function" && iuEvidenceRunEvidenceIntent("unknown_intent", {});
          result.unsupportedIntentHandled = !!(_invSim && _invSim.validInput === false && _invSim.validationErrors && _invSim.validationErrors.indexOf("unsupported_intent") >= 0);
          var _invSim2 = typeof iuEvidenceRunEvidenceIntent === "function" && iuEvidenceRunEvidenceIntent("spend_by_merchant", {});
          result.invalidInputHandled = !!(_invSim2 && _invSim2.validInput === false && _invSim2.validationErrors && _invSim2.validationErrors.length > 0);
          result.factLinkageTruthful = !!(_b68sim && _b68sim.sourceFactPackType === "spendSummary" && _b68sim.sourceFactPackAudit && _b68sim.sourceFactPackAudit.resultSource === "phase6.7");
          ...
          result.blockedAnswerFieldDisciplinePresent = false;
          result.answerSafetyMetadataPresent = false;
          result.unsupportedIntentHandled = false;
          result.invalidInputHandled = false;
        ...
        result.phase67FactsLayerPresent = false;
        result.spendSummaryFactPackPresent = false;
        result.phase68IntentContractPresent = false;
        result.supportedIntentsPresent = false;
        result.inputValidationPresent = false;
        result.intentRouterPresent = false;
        result.canonicalAnswerBundlePresent = false;
        result.safeAnswerFieldsPresent = false;
        result.blockedAnswerFieldDisciplinePresent = false;
        result.answerSafetyMetadataPresent = false;
  ...
  try {
    ...
    window.__iuEvidenceFactsLayer = iuEvidenceFactsLayer;
    window.IU_EVIDENCE_SUPPORTED_INTENTS = IU_EVIDENCE_SUPPORTED_INTENTS;
    window.iuEvidenceValidateIntentInput = iuEvidenceValidateIntentInput;
    window.iuEvidenceRunEvidenceIntent = iuEvidenceRunEvidenceIntent;
  ...
  function iuEvidenceUploadInit(){
    ...
    function showReviewPanel(result) {
      ...
      if (!result || typeof result !== "object") result = iuEvidenceOcrPipeline(iuEvidenceSimulatedRawOcr("receipt_photo", "unknown"), "receipt_photo");
      try {
        var d = window.__iuEvidenceDebug;
      ...
      try {
        ...
        var noOverwrite = cur && cur.ocrPass1Executed === true && dx && dx.ocrFinalCommitted === true && result && result.ocrPass1Executed !== true;
    ...
    function validateFile(file, acceptList) {
      ...
      var isImage = (file.type || "").indexOf("image/") === 0 || (file.name && /\.(png|jpe?g|gif|webp)$/i.test(file.name)) || (file.size > 0);
    ...
    function handleFile(file, kind) {
      ...
      iuEvidenceOcrHook(file, kind).then(function(result) {
        if (result == null || typeof result !== "object") {
          var committed = window.__iuEvidenceLastResult;
          var d = window.__iuEvidenceDebug;
          if (committed && committed.ocrPass1Executed === true && d && d.ocrFinalCommitted === true) { result = committed; } else {
            try { if (d) { d.failureReason = (d.failureReason || "workerOrRecognizeFail:hookResolvedUndefined"); d.rootRuntimeFailurePoint = (d.rootRuntimeFailurePoint || "hookResolvedUndefined"); d.rootCauseRemainingStopShip = (d.rootCauseRemainingStopShip || "hookResolvedUndefined"); d.ocrRunCompleted = true; d.ocrFinalState = "failure"; d.ocrResultSource = "safe_fallback"; } } catch (_) {}
            result = iuEvidenceOcrPipeline(iuEvidenceSimulatedRawOcr(kind, "unknown"), kind);
            result.usedInjectPath = false;
            result.proofStillDependsOnInjectedText = false;
