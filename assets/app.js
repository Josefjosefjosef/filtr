...
(function(){
  ...
  function normalizeSection(raw){
    ...
    if (k === 'ai' || k === 'shopping' || k === 'services') return 'media';
  ...
  function applyPanelFromUrl(){
    ...
    if (!panel) return;
    try{
      if (typeof window.iuOpenPanel === 'function') window.iuOpenPanel(panel);
    }catch(e){ try{ console.warn('[iu] panel open failed', panel, e); }catch{} }
  }
...
— infoUzel.cz`;
  ...
  function initNavRouter(){
    const feedEl = document.getElementById('feed');
    const viewEl = document.getElementById('iuRadioView');
    var rawSectionForBookmark = null;
    try{ rawSectionForBookmark = new URLSearchParams(window.location.search).get('section'); if (rawSectionForBookmark) rawSectionForBookmark = String(rawSectionForBookmark).trim().toLowerCase(); }catch{}
    try{
    ...
    applySectionFromURL();
    applyPanelFromUrl();
    if (rawSectionForBookmark && ['ai','shopping','services'].indexOf(rawSectionForBookmark) !== -1) {
      try{ const u = new URL(window.location.href); u.searchParams.set('section','media'); u.searchParams.set('panel', rawSectionForBookmark); history.replaceState(null,'',u.toString()); }catch{}
    }
  }
