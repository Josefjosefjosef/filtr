...
— infoUzel.cz`;
  ...
  function initNavRouter(){
    ...
    // Prefetch-on-intent + idle prewarm: cache and warmView (no visible state change, no data-view change).
    const viewHostCache = Object.create(null);
    const warmedViews = new Set();
    function warmView(viewKey) {
      const key = (viewKey && String(viewKey).trim().toLowerCase()) || '';
      if (!key || warmedViews.has(key)) return;
      const center = document.getElementById('iuCenterStage');
      if (!center) return;
      warmedViews.add(key);
      if (!viewHostCache[key]) {
        const host = center.querySelector('[data-view-host="' + key + '"]');
        if (host) viewHostCache[key] = host;
      }
    }
    function onLeftNavIntent(e) {
    ...
    if (leftRailEl) {
      try {
        leftRailEl.addEventListener('pointerenter', onLeftNavIntent, { passive: true });
        leftRailEl.addEventListener('focusin', onLeftNavIntent, { passive: true });
        leftRailEl.addEventListener('touchstart', onLeftNavIntent, { passive: true });
    ...
    try {
      ...
      scheduleIdle(function() {
        ...
        warmView('tvprogram');
        warmView('travel');
      }, typeof requestIdleCallback !== 'undefined' ? { timeout: 2000 } : undefined);
    } catch (_) {}
