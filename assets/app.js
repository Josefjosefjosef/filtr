  ...
  function initNavRouter(){
    ...
    function iuProjectsNavRouterTryOverlayBranch(){
      try {
        ...
        var stMindNav = false;
        try { stMindNav = !!(history.state && history.state.iu_mindmenu_overlay === true); } catch (_smn) {}
        var mindPending = false;
        try {
          var rawPend = localStorage.getItem("iuMindMenuReturnPendingV1");
          if (rawPend) {
            var objPend = JSON.parse(rawPend);
            var tsPend = objPend && Number(objPend.t);
            mindPending = Number.isFinite(tsPend) && tsPend > 0 && Date.now() - tsPend < 30 * 60 * 1000;
          }
        } catch (_mp) {}
        var mindArmed = false;
        try { mindArmed = sessionStorage.getItem("iuMindMenuReturnArmed") === "1"; } catch (_ma) {}
        if (hNav === "#iu-mindmenu" || stMindNav === true || mindPending || mindArmed) {
          try {
...
(function iuBootCalendarOverlayLazy() {
  "use strict";
  var CAL_URL = "./iu-calendar-overlay-v1.js?v=iu-calendar-persist-ack-v1-20260830-iu-cal-sheet-nav-stable-v1-20260904-calendar-nav-external-open-v1-20260916";
  var p = null;
