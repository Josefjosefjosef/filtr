...
function buildVideoAsArticleCard(it) {
  ...
  function iuMailboxRender(){
    try {
      ...
      const safeEscape = (s) => { try { return (typeof escapeHtml === "function" ? escapeHtml : String)(String(s || "")); } catch { return String(s || ""); } };
      items.forEach((it, i) => {
        ...
        row.innerHTML = minusHtml +
          `<button class="iu-mailbox-pill" type="button" data-mailbox-index="${i}" data-mailbox-open>${safeEscape(label)}</button>` +
      ...
      try { console.warn("[iuMailbox] render error:", err); } catch {}
    }
  }
  ...
  function iuMailboxesInit(){
    try {
      ...
      if (pillBtn) {
        ...
        } catch (err) { try { console.warn("[iuMailbox] click error:", err); } catch {} }
    });
    } catch (err) {
      try { console.warn("[iuMailbox] init error:", err); } catch {}
      window.__iuMailboxesInitDone = 0;
    }
  }
