...
(function(){
  ...
  function renderDayList(){
    ...
    const nowMs = Date.now();
    let nearestId = "";
    if (state.selectedDate === todayStr && items.length){
      for (let i = 0; i < items.length; i++){
        const ev = items[i];
        if (parseDateTime(ev.date, ev.time).getTime() >= nowMs){ nearestId = ev.id; break; }
      }
