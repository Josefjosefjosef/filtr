...
$port=9222
$url='https://infouzel.cz/projects/?debug=1&nosw=1&section=media'
...
try{
  function Get-TargetWsUrl {
    ...
    foreach($t in $list){
      if($t.type -eq 'page' -and $t.url -like "*infouzel.cz/projects/*"){ return $t.webSocketDebuggerUrl }
    }
    return $null
  }
  ...
  $expr=@'
(async () => {
  if(typeof window.__iuClearCLS === 'function') window.__iuClearCLS();
  await new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r)));
  const dump = (typeof window.__iuDumpCLS === 'function') ? window.__iuDumpCLS() : null;
  return { realTotal: dump?.realTotal ?? null };
})();
'@
  ...
  if($out.realTotal -ne 0){
    throw "CLS gate failed: realTotal=$($out.realTotal)"
  }
  "OK: CLS=0"
}finally{
