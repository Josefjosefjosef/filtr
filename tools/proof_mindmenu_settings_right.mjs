#!/usr/bin/env node
/**
 * Proof: MindMenu — gear inside row (row = visual pill), no DOM change.
 * Gate: gear inside .iu-mailbox-row bbox, position absolute/relative, right !== auto, CLS=0, console/pageerror=0.
 * Output: artifacts/PROOF_MINDMENU_SETTINGS_RIGHT.txt (or _PROD / AFTER_MERGE when PROOF_BASE_URL=prod).
 */
  ...
  try {
    ...
    const proofData = await page.evaluate(() => {
      ...
      for (const row of rowArr) {
        ...
        if (rGear.right > rRow.right || rGear.left < rRow.left) allInside = false;
    ...
    const lines = [
      ...
      "All icons inside row (pill) bounding box: " + (proofData.allIconsInsideRowBox ? "true" : "false"),
    ...
    if (isProd) writeArtifact("AFTER_MERGE_PROOF_MINDMENU_SETTINGS.txt", "PROOF: MindMenu gear inside row-pill — after merge\n" + lines.slice(1).join("\n"));
