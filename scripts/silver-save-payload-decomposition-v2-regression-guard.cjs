...
const SCRIPT = path.join(__dirname, "silver-save-payload-decomposition-v2-audit-v1.cjs");
function main() {
  let out = "";
  let exitCode = 0;
  try {
  ...
  console.log("=== SILVER_SAVE_PAYLOAD_DECOMPOSITION_V2_REGRESSION_GUARD ===");
  console.log("PASS_FAIL=" + (ok ? "PASS" : "FAIL"));
  console.log("=== END_SILVER_SAVE_PAYLOAD_DECOMPOSITION_V2_REGRESSION_GUARD ===");
  process.exit(ok ? 0 : 1);
}
