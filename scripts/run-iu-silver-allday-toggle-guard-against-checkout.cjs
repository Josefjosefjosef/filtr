...
/**
 * Run Silver all-day toggle guard against the current checkout on localhost.
 */
...
const PORT = Number(process.env.SILVER_ALLDAY_TOGGLE_GUARD_LOCAL_PORT || 8095);
...
async function main() {
  ...
  try {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(__dirname, "iu-silver-allday-toggle-guard-v1.cjs")],
      {
        stdio: "inherit",
        env: {
          ...process.env,
