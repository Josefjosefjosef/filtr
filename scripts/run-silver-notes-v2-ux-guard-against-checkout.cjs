...
/**
 * Run Silver Notes V2 UX guard against the current checkout on localhost.
 */
...
async function main() {
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [path.join(__dirname, "silver-notes-v2-ux-guard-v1.cjs")], {
      stdio: "inherit",
      env: { ...process.env, SILVER_NOTES_V2_UX_GUARD_URL: GUARD_URL },
    });
