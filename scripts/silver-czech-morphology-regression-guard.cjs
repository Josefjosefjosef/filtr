...
const SCRIPT = path.join(__dirname, "silver-czech-morphology-audit-v1.cjs");
function main() {
  let out = "";
  let exitCode = 0;
  try {
  ...
  console.log("=== SILVER_CZECH_MORPHOLOGY_REGRESSION_GUARD ===");
  console.log("PASS_FAIL=" + (ok ? "PASS" : "FAIL"));
  console.log("=== END_SILVER_CZECH_MORPHOLOGY_REGRESSION_GUARD ===");
  process.exit(ok ? 0 : 1);
}
