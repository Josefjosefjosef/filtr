...
const SCRIPT = path.join(__dirname, "silver-pathological-human-session-audit-v1.cjs");
function main() {
  let out = "";
  let exitCode = 0;
  try {
  ...
  console.log("=== SILVER_PATHOLOGICAL_HUMAN_SESSION_REGRESSION_GUARD ===");
  console.log("PASS_FAIL=" + (ok ? "PASS" : "FAIL"));
  console.log("=== END_SILVER_PATHOLOGICAL_HUMAN_SESSION_REGRESSION_GUARD ===");
  process.exit(ok ? 0 : 1);
}
