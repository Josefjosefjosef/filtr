...
/**
 * Run Silver speech-bubble stable/privacy (+4px down-shift) guard against checkout.
 */
const path = require("path");
const { spawn, spawnSync } = require("child_process");
const http = require("http");

const PORT = Number(process.env.SILVER_BUBBLE_GUARD_LOCAL_PORT || 8087);
...
async function main() {
  ...
  try {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(__dirname, "silver-mobile-tablet-bubble-stable-privacy-guard-v1.cjs")],
      {
        stdio: "inherit",
        env: {
          ...process.env,
