...
/**
 * Run real-route Datum/Čas geometry guard against the current checkout on localhost.
 */
...
const PORT = Number(process.env.IU_DATETIME_GUARD_LOCAL_PORT || 8094);
...
async function main() {
  ...
  try {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(__dirname, "iu-datetime-real-route-geometry-guard-v1.cjs")],
      {
        stdio: "inherit",
        env: {
          ...process.env,
          IU_DATETIME_GUARD_URL: GUARD_URL,
