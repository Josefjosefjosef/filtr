/**
 * Silver Real UX audit v2 — 30k temporal + dirty Czech + cross-module stress (READ engine from assets/app.js only).
 * harness_id: audit_silver_real_ux_v2_30000
 */
...
const HARNESS_ID = "audit_silver_real_ux_v2_30000";
const FIXED_NOW_ISO = "2026-05-04T12:00:00";
const REPO = path.resolve(__dirname, "..");
const REPORT_JSON = path.join(__dirname, "silver-real-ux-v2-30000-report.json");
const TOTAL = 30000;
...
function buildSeed() {
  ...
  const tasks = [
    ...
    { id: "t3", title: "koupit mléko", status: "todo", dueAt: addDaysIso(TODAY, 3), note: "dokoupit do pátku", priority: "medium", createdAt: 1, updatedAt: 1 },
    { id: "t_kytky", title: "koupit kytky", status: "todo", dueAt: ZITRA, note: "k narozeninám", priority: "medium", createdAt: 1, updatedAt: 1 },
...
function caseEvalAs(c) {
  return (c.meta && c.meta.eval_as) || "";
}
...
function ctxForCase(c) {
  const g = c.group;
  const ev = caseEvalAs(c);
  const m = c.meta || {};
  if (g === "calendar_query" || g === "task_query" || g === "note_query" || g === "multi_intent") return ctxQuery();
  if (g === "ambiguity_safety") {
    if (ev === "safety_negation" || ev === "temporal_reasoning") return ctxQuery();
    if (ev === "dirty_mobile_czech" && m.is_query) return ctxQuery();
    if (ev === "reminder_intent" && m.needs_seed) return ctxQuery();
    return ctxEmpty();
  }
  if (ev === "reminder_intent" && m.needs_seed) return ctxQuery();
  return ctxEmpty();
...
function routingCategoryForEngine(c) {
  ...
  const ev = caseEvalAs(c);
  const m = c.meta || {};
  if (ev === "dirty_mobile_czech" && m.underlying) return m.underlying;
  if (ev === "temporal_reasoning") return "calendar_query";
  if (ev === "reminder_intent") return m.reminder_target === "task" ? "task_write" : "calendar_write";
  return g;
}
...
function gitTrackedClean() {
  try {
    ...
    const allow = [
      ...
      "scripts/silver-real-ux-v2-30000-report.json",
...
function semanticGroupForEval(c) {
  ...
  if (ev === "temporal_reasoning") return "calendar_query";
  if (ev === "dirty_mobile_czech" && m.underlying) return m.underlying;
  if (ev === "safety_negation") return m.semantic_as ? m.semantic_as : "calendar_query";
  if (ev === "update_vs_create") return "update_vs_create";
  if (ev === "reminder_intent") return "reminder_intent";
  return c.group;
}
...
function evaluateCore(c, turn) {
  ...
  if (semG === "calendar_write" || c.group === "calendar_write" || (caseEvalAs(c) === "dirty_mobile_czech" && c.meta && c.meta.underlying === "calendar_write")) {
  ...
  if (caseEvalAs(c) === "safety_negation") {
  ...
  if (caseEvalAs(c) === "update_vs_create") {
  ...
  if (caseEvalAs(c) === "reminder_intent") {
  ...
  const calWriteHarness =
    semG === "calendar_write" || (caseEvalAs(c) === "dirty_mobile_czech" && c.meta && c.meta.underlying === "calendar_write");
  ...
  if (auditIntent === "unknown" || eng === "clarification") {
    ...
    if (
      caseEvalAs(c) === "temporal_reasoning" ||
  ...
  if ((c.group.indexOf("_query") > 0 || caseEvalAs(c) === "safety_negation" || caseEvalAs(c) === "temporal_reasoning") && hasNegWrite(folded)) {
    if (turn.processingState === "READY_TO_SAVE" || eng === "calendar.create" || eng === "tasks.create" || eng === "notes.create") {
...
function extraQualityTags(c, turn, ev) {
  ...
  if (c.group === "calendar_write" || (caseEvalAs(c) === "dirty_mobile_czech" && c.meta && c.meta.underlying === "calendar_write")) {
  ...
  if (c.group === "task_write" || (caseEvalAs(c) === "dirty_mobile_czech" && c.meta && c.meta.underlying === "task_write")) {
    const tc = titleCleanForDraft(turn, folded);
    if (!tc.ok && !tc.skip) tags.push("dirty_title");
    if (eng === "calendar.create") tags.push("task_calendar_confusion");
  }
  if (c.group === "note_write" || (caseEvalAs(c) === "dirty_mobile_czech" && c.meta && c.meta.underlying === "note_write")) {
    const tc = titleCleanForDraft(turn, folded);
    if (!tc.ok && !tc.skip) tags.push("dirty_title");
  }
...
function buildCases() {
  ...
  for (let i = 0; i < 3200; i++) {
    ...
    const lines = [
      ...
      `${neg}Cau cau pls nahod mi meeting zitra odpoledne s tim typem z prahy a nezapomen adresu.`,
      `${neg}Mobil chaos: zitra?? schuzka pravnik 15:30 korunní poznamka smlouva obcanka nic jinyho neukladej do ukolu jen kalendář.`
    ];
  ...
  for (let i = 0; i < 400; i++) {
    ...
    push({
      ...
      input: diacVariant(i, calTRem[i % calTRem.length]),
      expectedIntent: "calendar.create",
      meta: { eval_as: "reminder_intent", reminder_kind: "calendar", reminder_target: "calendar", needs_seed: r === 1 }
    });
  }
  const missRem = ["Připomeň mi právníka.", "Připomeň mi schůzku s Novákem."];
  for (let i = 0; i < 200; i++) {
    push({
      group: "calendar_write",
      input: diacVariant(i, missRem[i % missRem.length]),
      expectedIntent: "unknown",
      meta: { eval_as: "reminder_intent", reminder_kind: "missing_time", reminder_target: "calendar", needs_seed: false }
    });
  }
  const dirtyCalW = [
  ...
  for (let i = 0; i < 200; i++) {
    push({
      group: "calendar_write",
      input: dirtyCalW[i % dirtyCalW.length],
      expectedIntent: "calendar.create",
      meta: {
        eval_as: "dirty_mobile_czech",
        underlying: "calendar_write",
        secondary_tags: ["calendar_write"],
        embed_needle: "smlouv"
      }
    });
  }
  ...
  for (let i = 0; i < 4000; i++) {
    ...
    const futureLines = [
      ...
      `Co mám pozítří v kalendáři?`,
      `Kolik schůzek mám pozítří?`,
    ...
    const pastLines = [
      ...
      `V kolik jsem měl včera schůzku?`,
  ...
  for (let i = 0; i < 800; i++) {
    ...
    push({
      ...
      meta: {
        eval_as: "temporal_reasoning",
        time_scope: counting ? undefined : ts,
        counting_query: counting,
        folded_probe: foldCs(base)
      }
    });
  }
  const dirtyCalQOnly = [
  ...
  for (let i = 0; i < 200; i++) {
    const dq = dirtyCalQOnly[i % dirtyCalQOnly.length];
    push({
      group: "calendar_query",
      input: dq,
      ...
      meta: {
        eval_as: "dirty_mobile_czech",
        underlying: "calendar_query",
        is_query: true,
        secondary_tags: ["calendar_query"],
        time_scope: "future",
        folded_probe: foldCs(dq)
      }
    });
  }
  ...
  const taskTRem = ["Připomeň mi koupit mléko.", "Připomeň mi zavolat Petrovi.", "Připomeň mi zaplatit fakturu."];
  for (let i = 0; i < 500; i++) {
    push({
      group: "task_write",
      input: diacVariant(i, taskTRem[i % taskTRem.length]),
      expectedIntent: "task.create",
      meta: { eval_as: "reminder_intent", reminder_kind: "task", reminder_target: "task" }
    });
  }
  ...
  for (let i = 0; i < 6000; i++) {
    ...
    const active = [
      ...
      `Kdy mám kupovat kytky?`,
      `Dokdy mám koupit mléko?`,
      `Najdi mi úkoly co mám v kalendáři`,
      `Koukni do ukolu co mam dneska udelat nic neukladej`,
      `vypis hotovy ukoly`,
      `ukoly na pozitri prosim jen cti`
    ];
    const done = [
    ...
    const longChaos =
      "Hele jako fakt potřebuju vědět jestli mám v úkolech něco na zítra a jestli to není spíš v kalendáři protože jsem si to možná špatně uložil minule na mobilu když jsem spěchal z busu.";
    let inp;
    let meta = {};
    const pick = i % 13;
    if (pick === 10) {
      inp = diacVariant(i, longChaos + (i % 4 === 0 ? " " + "blabla ".repeat(15) : ""));
      meta = { task_query_kind: "unspecified", clutter_penalty_probe: true };
    } else if (pick % 3 === 0) {
      inp = diacVariant(i, active[pick % active.length]);
      meta = { task_query_kind: "active" };
    } else if (pick % 3 === 1) {
      inp = diacVariant(i, done[pick % done.length]);
      meta = { task_query_kind: "completed" };
    } else {
      inp = diacVariant(i, undone[pick % undone.length]);
  ...
  for (let i = 0; i < 3500; i++) {
    ...
    const lines = [
      ...
      `${neg}Ne do kalendáře, jen do poznámek: heslo od trezoru je 9999.`
    ];
  ...
  for (let i = 0; i < 4000; i++) {
    ...
    const lines = [
      ...
      `Jen čti poznámky, nic nevytvářej, co mám o PINu?`
    ];
  ...
  for (let i = 0; i < 500; i++) {
    ...
    push({
      group: "ambiguity_safety",
      input: t.inp,
      expectedIntent: t.exp,
      meta: { eval_as: "safety_negation", semantic_as: t.sem }
    });
  }
  ...
  for (let i = 0; i < 400; i++) {
    ...
    push({
      group: "ambiguity_safety",
      input: diacVariant(i, lines[i % lines.length]),
      expectedIntent: "non_create_ok",
      meta: { eval_as: "update_vs_create" }
    });
  }

  for (let i = 0; i < 300; i++) {
    const cross = [
      { inp: "Ne do poznámek, jen mi řekni co mám zítra v kalendáři.", exp: "calendar.query", sem: "calendar_query" },
      { inp: "Ne do kalendáře, co mám v úkolech na zítra?", exp: "task.query", sem: "task_query" },
      { inp: "Nic nevytvářej, jen čti: mám něco pozítří?", exp: "calendar.query", sem: "calendar_query" }
    ];
    const t = cross[i % cross.length];
    push({
      group: "ambiguity_safety",
      input: t.inp,
      expectedIntent: t.exp,
      meta: { eval_as: "safety_negation", semantic_as: t.sem, cross_module: true }
    });
  }

  const dirtyAmb = ["hele neukladej nic jen mi rekni kolikrat sem byl u zubara letos", "jen cti ukoly ne do kalendare nic neukladej"];
  for (let i = 0; i < 300; i++) {
    const inp = dirtyAmb[i % dirtyAmb.length];
    const isTask = /ukol/i.test(inp);
    push({
      group: "ambiguity_safety",
      input: inp,
      ...
      meta: {
        eval_as: "dirty_mobile_czech",
        underlying: isTask ? "task_query" : "calendar_query",
        is_query: true,
        secondary_tags: [isTask ? "task_query" : "calendar_query"]
      }
    });
  }
...
function rootCauseLine(cat) {
  const c = String(cat || "");
  if (/write_when_negated|query_created|false_write|dangerous|negative_instruction/.test(c)) return "Safety boundary: read vs write routing.";
  if (/temporal|past_query|future_query/.test(c)) return "Temporal interpretation in user-facing reply.";
  if (/intent_fail|calendar_vs|note_vs|wrong_collection|query_wrong/.test(c)) return "Intent routing or collection disambiguation.";
  if (/disambigu|clutter|dirty|quality|embedded|standalone/.test(c)) return "Draft hygiene / disambiguation UX.";
  return "Semantic or harness expectation gap.";
}
...
function gitNameOnlyPorcelainTracked() {
  try {
    const o = execSync("git status --porcelain", { cwd: REPO, encoding: "utf8" });
    return o
      .split(/\r?\n/)
...
function main() {
  ...
  let writeFromQueryCount = 0;
  let writeWhenNegatedCount = 0;
  let updateRiskNewCreateCount = 0;
  let calendarVsTaskConfusionCount = 0;
  let calendarVsNoteConfusionCount = 0;
  let taskVsNoteConfusionCount = 0;
  let storageDisambiguationWrongCount = 0;
  let temporalGapCount = 0;
  let queryRelevanceFailCount = 0;
  let dirtyTitleCount = 0;
  let clutterPenaltyCount = 0;
  let missingDatetimeHandlingCount = 0;
  let routingConfusionNumerator = 0;
  ...
  for (let ci = 0; ci < cases.length; ci++) {
    ...
    if (c.group.indexOf("_write") > 0 || c.group === "calendar_write" || c.group === "multi_intent" || caseEvalAs(c) === "reminder_intent") {
      writeDenom++;
    ...
    if (
      ...
      c.group === "calendar_query" ||
      caseEvalAs(c) === "safety_negation" ||
      caseEvalAs(c) === "temporal_reasoning" ||
      (c.group === "ambiguity_safety" && caseEvalAs(c) !== "update_vs_create")
    ) {
      queryDenom++;
    ...
    if (
      (c.group === "calendar_query" || c.group === "task_query" || c.group === "note_query") &&
      createLike
    ) {
      writeFromQueryCount++;
    }
    if (caseEvalAs(c) === "safety_negation" && createLike) writeWhenNegatedCount++;
    if (ev.cat === "update_risk_new_create") updateRiskNewCreateCount++;
    if (ev.cat === "calendar_vs_task_confusion") calendarVsTaskConfusionCount++;
    if (ev.cat === "wrong_collection") calendarVsNoteConfusionCount++;
    if (ev.cat === "note_vs_task_confusion") taskVsNoteConfusionCount++;
    if (ev.cat === "unnecessary_disambiguation") storageDisambiguationWrongCount++;
    if (ev.cat === "future_query_returns_past" || ev.cat === "past_query_ignored") temporalGapCount++;
    if (ev.cat === "false_negative" || ev.cat === "query_wrong_dataset") queryRelevanceFailCount++;
    if (qtags.indexOf("dirty_title") >= 0) dirtyTitleCount++;
    if (c.meta && c.meta.clutter_penalty_probe && !fullPass) clutterPenaltyCount++;
    if (ev.cat === "reminder_missing_time_not_clarified") missingDatetimeHandlingCount++;
    if (!corePass && /intent_fail|calendar_vs_task|wrong_collection|note_vs_task|query_wrong|unnecessary_disambiguation/.test(String(ev.cat || "")))
      routingConfusionNumerator++;

    if (c.group === "calendar_query" && c.meta && c.meta.time_scope) {
    ...
    if (
      ...
      (caseEvalAs(c) === "dirty_mobile_czech" && c.meta && c.meta.underlying === "calendar_write") ||
      c.group === "task_write" ||
      (caseEvalAs(c) === "dirty_mobile_czech" && c.meta && c.meta.underlying === "task_write") ||
      c.group === "note_write" ||
      (caseEvalAs(c) === "dirty_mobile_czech" && c.meta && c.meta.underlying === "note_write")
    ) {
      const tc = titleCleanForDraft(turn, folded);
      if (!tc.skip) {
        titleDenom++;
  ...
  const top25 = topClusters.map((x) => {
    ...
    return {
      ...
      root_cause_guess: rootCauseLine(cat),
  ...
  const top10Clusters = topClusters.slice(0, 10).map((x) => {
    const parts = x.k.split("|");
    const cat = parts[0];
    const ex = clusterExamples.get(x.k) || [];
    const first = ex[0] || {};
    return {
      ...
      count: x.n,
      example_inputs: ex.map((e) => e.input),
      expected: first.expected_summary || "",
      actual: first.actual_summary || "",
      root_cause: rootCauseLine(cat)
    };
  });
  ...
  const routingConfusionRate = pct(routingConfusionNumerator, TOTAL);
  ...
  const riskAssessment =
    dangerousWriteCount > 0
      ? "P0 safety signals present (writes on negated read paths)."
      : falseWriteCount > 0
        ? "P0 false writes on query-shaped prompts."
        : parseFloat(overallAcc) < 75
          ? "Overall routing quality below 75% — prioritize top cluster."
          : "No P0 write-safety flags in harness; remaining gaps are P1/P2 UX.";
  ...
  const report = {
    ...
    ambiguity_safety_accuracy: pct(
      byGroup.ambiguity_safety.pass,
      byGroup.ambiguity_safety.pass + byGroup.ambiguity_safety.fail
    ),
    routing_confusion_rate: routingConfusionRate,
    ...
    write_from_query_count: writeFromQueryCount,
    write_when_negated_count: writeWhenNegatedCount,
    calendar_vs_task_confusion_count: calendarVsTaskConfusionCount,
    calendar_vs_note_confusion_count: calendarVsNoteConfusionCount,
    task_vs_note_confusion_count: taskVsNoteConfusionCount,
    storage_disambiguation_wrong_count: storageDisambiguationWrongCount,
    temporal_gap_count: temporalGapCount,
    query_relevance_fail_count: queryRelevanceFailCount,
    dirty_title_count: dirtyTitleCount,
    clutter_penalty_count: clutterPenaltyCount,
    missing_datetime_handling_count: missingDatetimeHandlingCount,
    update_vs_create_confusion_count: updateRiskNewCreateCount,
    ...
    recommended_next_cluster: recommended_next_fix_cluster,
    recommended_next_reason: recommended_next_fix_reason,
  ...
  const t1 = top10Clusters[0] || {};
  const t2 = top10Clusters[1] || {};
  const t3 = top10Clusters[2] || {};
  const t4 = top10Clusters[3] || {};
  const t5 = top10Clusters[4] || {};
  const t6 = top10Clusters[5] || {};
  const t7 = top10Clusters[6] || {};
  const t8 = top10Clusters[7] || {};
  const t9 = top10Clusters[8] || {};
  const t10 = top10Clusters[9] || {};
  const exJoin = (t) => (t.example_inputs || []).slice(0, 3).join(" | ");
  const oneLine = (s) => String(s || "").replace(/\s+/g, " ").trim();

  const summaryLines = [
    "=== SILVER_REAL_UX_V2_30000_AUDIT_RESULT ===",
    "main_commit=" + mainCommit,
    "engine_changed=NO",
    "assets_app_changed=NO",
    "changed_files=" + changedFiles,
    "total_cases=" + TOTAL,
    "overall_accuracy=" + overallAcc + "%",
    "calendar_write_accuracy=" +
      pct(byGroup.calendar_write.pass, byGroup.calendar_write.pass + byGroup.calendar_write.fail) +
      "%",
    "calendar_query_accuracy=" +
      pct(byGroup.calendar_query.pass, byGroup.calendar_query.pass + byGroup.calendar_query.fail) +
      "%",
    "task_write_accuracy=" + pct(byGroup.task_write.pass, byGroup.task_write.pass + byGroup.task_write.fail) + "%",
    "task_query_accuracy=" + pct(byGroup.task_query.pass, byGroup.task_query.pass + byGroup.task_query.fail) + "%",
    "note_write_accuracy=" + pct(byGroup.note_write.pass, byGroup.note_write.pass + byGroup.note_write.fail) + "%",
    "note_query_accuracy=" + pct(byGroup.note_query.pass, byGroup.note_query.pass + byGroup.note_query.fail) + "%",
    "multi_intent_accuracy=" + pct(byGroup.multi_intent.pass, byGroup.multi_intent.pass + byGroup.multi_intent.fail) + "%",
    "ambiguity_safety_accuracy=" +
      pct(byGroup.ambiguity_safety.pass, byGroup.ambiguity_safety.pass + byGroup.ambiguity_safety.fail) +
      "%",
    "dangerous_write_count=" + dangerousWriteCount,
    "false_write_count=" + falseWriteCount,
    "query_created_write_count=" + queryCreatedWriteCount,
    ...
    "===== END_SILVER_REAL_UX_V2_30000_AUDIT_RESULT ==="
  ];
