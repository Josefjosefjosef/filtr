...
const PORT = String(process.env.IU_GUARD_PORT || "9877");
...
async function main() {
  ...
  for (let i = 0; i < 80; i++) {
    await sleep(50);
    if (/127\.0\.0\.1/.test(boot) || /8890|9877/.test(boot)) break;
  }
  ...
  async function runPage(suffix) {
    ...
    const snap = await page.evaluate(() => {
      ...
      const texts = Array.from(document.querySelectorAll("#feed a.iuCardTitle")).map((a) =>
        String(a.textContent || "").trim(),
      );
      // Match silver-prod / weather guards: absolute rail .left is not "shift" (padding/grid).
      let railShift =
        typeof window.__iuRailShiftProbe === "number" ? window.__iuRailShiftProbe : 0;
      const de = document.documentElement;
      const overflowX = de.scrollWidth > de.clientWidth + 2;
      return { topic, hrefs, texts, railShift: Math.round(railShift), overflowX };
    });
    await ctx.close();
    return snap;
  }
  ...
  try {
    ...
    const byTs = (a) => {
      const p = Date.parse(a.publishedAt || a.pubDate || a.date || "") || 0;
      return p;
    };
    ...
    const overflowX = def.overflowX || zpravy.overflowX || sport.overflowX || finance.overflowX;

    const PASS_DEFAULT_GLOBAL = def.topic === "" && defaultHasCrossSection && def.hrefs.length > 40;
    const PASS_ZPRAVY_NOT_FULL_SPORT = zpravy.topic === "zpravy" && zpravyBlocksPureSportHost;
    ...
    const report = {
      ...
      overflowX,
      railShift,
      defaultTopicParam: def.topic,
      defaultFeedCount: def.hrefs.length,
      zpravyFeedCount: zpravy.hrefs.length,
      topTitlesDefault: def.texts.slice(0, 8),
    };
    ...
    if (!allPass) {
      process.stderr.write("home-feed-default-global-guard: FAIL\n");
      process.exitCode = 1;
    }
  } finally {
    try {
