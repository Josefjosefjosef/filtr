/**
 * RHC3 cluster diagnostic (read-only): rhc3_filler_note_query (filler_speech / note_query_chaos).
 ...
 * title_pollution_count: engine draft/title fields echo spoken filler tokens (diagnostic signal).
...
const REPORT_JSON = path.join(__dirname, "silver-rhc3-filler-note-query-diagnostic-report.json");
...
const EXPECTED_MAIN_COMMIT = "c28748054a4fe8e3533e755414e45276645879bf";

const TARGET_CLUSTER = "rhc3_filler_note_query";
const RANDOM_SAMPLE_SEED = 0x66696c6e;
...
function isChaoticMutationSurface(c) {
  const mask = (c.mutation_mask || 0) >>> 0;
  if (noiseBitsPopcount(c) >= 3) return true;
  if ((mask & core.M.NEGATION_OVERLAY) !== 0) return true;
  if ((mask & core.M.AMBIGUITY_OVERLAY) !== 0) return true;
  return false;
}

/** Structural chaos beyond filler prefix/suffix / typos: partial ref, mobile, spoken compress (not hesitation alone). */
function fillerStructuralDisruption(c) {
  const mask = (c.mutation_mask || 0) >>> 0;
  return (
...
function fillerHesitationSurface(c) {
...
function hasNoteQueryMarkersFolded(fold) {
  return /\bpoznam|not(e|a)\b/i.test(String(fold || ""));
}

/** List/read DNA for note corpus (Czech): „kde mám“, „mrkni“, „co mám“, „ohledně … poznámek“, etc. */
function hasNoteQueryListCueFolded(fold) {
  const f = String(fold || "");
  if (!hasNoteQueryMarkersFolded(f)) return false;
  /** HESITATION infix can insert tokens between „kde“ and „mám“ (e.g. kde ehm vlastně mám). */
...
/** Engine-side: filler tokens copied into draft fields (pollution vs user input). */
function detectDraftTitlePollution(turn) {
  const d = turn.draft || {};
  const blob = [d.title, d.note, d.silverNoteText, d.searchQuery, d.query]
    .filter(Boolean)
    .map((x) => String(x))
    .join(" ");
  if (!blob.trim()) return false;
  const f = foldCs(blob);
  return TITLE_POLLUTION_RE.test(f);
}

/**
 * Mutually exclusive primary + subcluster for rhc3_filler_note_query fails.
 * @returns {{ primary: string, subcluster: string }}
 */
function classifyFillerNoteQuery(c, turn, ev) {
  ...
  const partialRef = hasPartialRefSurface(c);
  const noteMarkers = hasNoteQueryMarkersFolded(fold);
  const listCue = hasNoteQueryListCueFolded(fold);
  ...
  if (cat === "raw_response_empty") {
    const readLike = eng === "notes.read" || eng === "global.search";
    if (readLike && !drafty) {
  ...
  if (cat === "unnecessary_disambiguation") {
    return { primary: "HARNESS_LABEL_PROBLEM", subcluster: "storage_disambig_on_note_query" };
  }

  if (cat === "false_negative") {
    return { primary: "HARNESS_LABEL_PROBLEM", subcluster: "note_query_semantic_false_negative" };
  }
  ...
  if (exp === "note.query") {
    if (drafty) {
      return { primary: "TRUE_ENGINE_FAIL", subcluster: "query_surface_routed_to_create:" + eng };
    }
    ...
    if (eng === "clarification" || eng === "unknown" || auditIntent === "unknown") {
      ...
      return { primary: "HARNESS_LABEL_PROBLEM", subcluster: "clear_surface_should_route_notes_read" };
    }
...
function main() {
  ...
  if (!git.ok) {
    console.log("=== RHC3_FILLER_NOTE_QUERY_DIAGNOSTIC_ABORT ===");
  ...
  for (let i = 0; i < cases.length; i++) {
    ...
    if (ev.pass) {
      ...
      clusterFailCount++;
      const cls = classifyFillerNoteQuery(c, turn, ev);
      countKey(primaryCounts, cls.primary);
      countKey(subclusterCounts, cls.subcluster);
      countKey(clusterFailHarnessCat, String(ev.cat || "unknown"));
      clusterFailById.set(c.id, { c, turn, ev, cls });
      if (detectDraftTitlePollution(turn)) titlePollutionCount++;
    }
  }
  ...
  } else if (wrongModuleCount > trueEngineFailCount && wrongModuleCount > retrievalMissCount) {
    recommendedNextTask =
      "engine_routing: reduce task/calendar reads on filler note_query surfaces (cross-module vs notes.read).";
  } else if (retrievalMissCount > harnessProblemCount && retrievalMissCount > ambiguousInputCount) {
    recommendedNextTask =
      "engine_retrieval: notes.read / global.search empty or off-topic for seeded note snapshots (filler_speech family).";
  } else if (trueEngineFailCount > safeClarificationOkCount * 3 && wrongModuleCount === 0 && retrievalMissCount === 0) {
    recommendedNextTask =
      "engine_routing: dominant fails are intent_fail / create-like on filler note_query — read-vs-create bug triage.";
  } else if (harnessProblemCount > ambiguousInputCount) {
    recommendedNextTask =
      "scripts-only: align noteQuerySemantic / gold for clear-surface false negatives or storage disambig harness.";
  } else if (ambiguousInputCount > trueEngineFailCount && clusterFailCount > 0) {
    recommendedNextTask =
      "corpus+harness: document PASS scope for heavy filler / partial_ref note_query chaos (no engine NLP).";
  } else if (titlePollutionCount > clusterFailCount * 0.25 && clusterFailCount > 0) {
    recommendedNextTask =
      "engine_ux: reduce spoken filler echo into draft title/search fields on note reads (title pollution signal).";
  } else if (clusterFailCount === 0 && mustFixEngineCount === 0) {
    recommendedNextTask = "none: rhc3_filler_note_query cluster green under harness + engine baseline.";
  }

  const gitCleanAll = gitStatusShortClean();

  const lines = [
    "=== RHC3_FILLER_NOTE_QUERY_DIAGNOSTIC_RESULT ===",
    ...
    "=== END_RHC3_FILLER_NOTE_QUERY_DIAGNOSTIC_RESULT ==="
  ];
  ...
  for (let ii = 0; ii < inspected.length && examples.length < 28; ii++) {
    ...
    examples.push({
      ...
      title_pollution: detectDraftTitlePollution(turn),
  ...
  const reportObj = {
    ...
    cluster_fail_harness_cat: clusterFailHarnessCat,
...
module.exports = {
  TARGET_CLUSTER,
  classifyFillerNoteQuery,
  EXPECTED_MAIN_COMMIT,
  hasNoteQueryListCueFolded,
  detectDraftTitlePollution
};
