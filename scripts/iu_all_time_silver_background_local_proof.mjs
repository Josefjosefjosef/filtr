/**
 * LOCAL_ALL_TIME_SILVER_BACKGROUND_ONLY_PROOF
 * node scripts/iu_all_time_silver_background_local_proof.mjs (repo root)
 */
...
async function probeMode(page, mode, consoleErrors, withGreeting) {
  ...
  const greetOk = String(phrase).indexOf(mode.greetingNeedle) >= 0;
  const premium = await silverPremiumActive(page);
  const inpStable = await fingerprintInputTwice(page);
  const imgStable = await fingerprintImgTwice(page);
  const layStable = await fingerprintLayoutTwice(page);
  const snap = await snapMetrics(page);
  const ce = Math.max(0, consoleErrors.length - err0);
  const row = {
    silverBackgroundPremiumActive: premium,
    forbiddenInputChanged: !inpStable,
    forbiddenImgChanged: !imgStable,
    forbiddenLayoutChanged: !layStable,
    CLS: snap.clsSum,
    overflowX: snap.overflowX,
    railShift: snap.railShift,
    consoleErrorsCount: ce
  };
  if (withGreeting) row.greeting = greetOk ? mode.greetingNeedle : phrase;
  return row;
}
...
async function runViewport(page, vw, vh, consoleErrors, withGreetings) {
  ...
  for (let i = 0; i < MODES.length; i++) {
    const m = MODES[i];
    out[m.key] = await probeMode(page, m, consoleErrors, withGreetings);
  }
  await page.evaluate((h) => {
    if (typeof window.iuSilverWelcomeRefresh === "function") window.iuSilverWelcomeRefresh({ hour: h });
  }, 7);
...
async function main() {
  ...
  const v390 = await runViewport(page, 390, 844, consoleErrors, true);
  const v768 = await runViewport(page, 768, 900, consoleErrors, false);
  ...
  const layoutChanged = Object.values(v390.rows).some((r) => r.forbiddenLayoutChanged) ||
    Object.values(v768.rows).some((r) => r.forbiddenLayoutChanged);

  const lines = ["=== LOCAL_ALL_TIME_SILVER_BACKGROUND_ONLY_PROOF ===", "viewport_390:"];
  for (let i = 0; i < MODES.length; i++) {
    const k = MODES[i].key;
    const r = v390.rows[k];
    lines.push(`  ${k}:`);
    lines.push(`    greeting: ${r.greeting}`);
    ...
    lines.push(`    forbiddenLayoutChanged: ${r.forbiddenLayoutChanged}`);
  ...
  for (let i2 = 0; i2 < MODES.length; i2++) {
    ...
    lines.push(`    forbiddenLayoutChanged: ${r2.forbiddenLayoutChanged}`);
  ...
  lines.push("=== END_LOCAL_ALL_TIME_SILVER_BACKGROUND_ONLY_PROOF ===");
  ...
  for (let j = 0; j < MODES.length; j++) {
    if (failRow(v390.rows[MODES[j].key])) fail = true;
    if (failRow(v768.rows[MODES[j].key])) fail = true;
  }
  if (v390.otherBoxesChanged || v768.otherBoxesChanged) fail = true;
  if (layoutChanged) fail = true;
  for (let g = 0; g < MODES.length; g++) {
    const gr = v390.rows[MODES[g].key].greeting;
    if (String(gr) !== MODES[g].greetingNeedle) fail = true;
  }
