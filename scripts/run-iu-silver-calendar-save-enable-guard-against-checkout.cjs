...
/**
 * Run Silver calendar Save-enable guard against the current checkout on localhost.
 */
...
const PORT = Number(process.env.SILVER_CAL_SAVE_ENABLE_GUARD_LOCAL_PORT || 8098);
...
async function main() {
  ...
  try {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(__dirname, "iu-silver-calendar-save-enable-guard-v1.cjs")],
      {
        stdio: "inherit",
        env: {
          ...process.env,
