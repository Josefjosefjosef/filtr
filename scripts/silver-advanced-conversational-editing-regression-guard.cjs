...
const SCRIPT = path.join(__dirname, "silver-advanced-conversational-editing-audit-v1.cjs");
function main() {
  let out = "";
  let exitCode = 0;
  try {
  ...
  console.log("=== SILVER_ADVANCED_CONVERSATIONAL_EDITING_REGRESSION_GUARD ===");
  console.log("PASS_FAIL=" + (ok ? "PASS" : "FAIL"));
  console.log("=== END_SILVER_ADVANCED_CONVERSATIONAL_EDITING_REGRESSION_GUARD ===");
  process.exit(ok ? 0 : 1);
}
