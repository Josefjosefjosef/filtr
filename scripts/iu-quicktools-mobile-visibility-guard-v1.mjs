...
async function seedDefaultConfig(page) {
  await page.evaluate(({ order, customId }) => {
    const cfg = {
      version: 2,
      order: order.slice(),
      visible: order.slice(),
    ...
    cfg.order.splice(cfg.order.length - 1, 0, customId);
    cfg.visible.splice(cfg.visible.length - 1, 0, customId);
    localStorage.setItem("infouzel_quicktools", JSON.stringify(cfg));
  }, { order: DEFAULT_ORDER, customId: CUSTOM_ID });
}
