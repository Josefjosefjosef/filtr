...
/**
 * Run Silver home date/time input-fit geometry guard against the current checkout on localhost.
 */
...
const PORT = Number(process.env.SILVER_DATE_TIME_FIT_GUARD_LOCAL_PORT || 8093);
// Local static maps `/` → projects/index.html; never use /projects/ as the hub URL.
...
async function main() {
  ...
  try {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(__dirname, "silver-home-date-time-input-fit-guard-v1.cjs")],
      {
        stdio: "inherit",
        env: {
          ...process.env,
