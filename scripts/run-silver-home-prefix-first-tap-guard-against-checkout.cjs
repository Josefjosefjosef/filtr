...
/**
 * Run Silver home prefix first-tap guards against the current checkout on localhost.
 */
...
async function main() {
  const staticResult = spawnSync(
    process.execPath,
    [path.join(__dirname, "silver-home-prefix-first-tap-static-guard-v1.cjs")],
    { stdio: "inherit" }
  );
  ...
  try {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(__dirname, "silver-home-prefix-first-tap-guard-v1.cjs")],
      {
        stdio: "inherit",
        env: { ...process.env, SILVER_HOME_PREFIX_FIRST_TAP_GUARD_URL: GUARD_URL },
      }
    );
